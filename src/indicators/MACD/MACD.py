...
class MACD:
	...
	def __init__(
		...
		self.elements = dict({
							...
							'dataset_15M': parameters.elements['dataset_15M'],
							'dataset_1H': parameters.elements['dataset_1H'],
							'dataset_4H': parameters.elements['dataset_4H'],
							'dataset_1D': parameters.elements['dataset_1D'],
	...
	def LastSignal(
		...
		try:
			...
			if False:#GL_Results_buy_primary['permit'][0] == True:
		...
		try:
			...
			if False:#GL_Results_buy_secondry['permit'][0] == True:
		...
		try:
			if False:#GL_Results_sell_primary['permit'][0] == True:
		...
		if (
			...
			(len(dataset_5M_real[symbol]['close']) - 1 - lst_idx_buy_primary) >= 5
			):
			...
			if (
				...
				plt.plot(dataset_5M_real[symbol]['low'][int(signal_buy_primary['index_back'][int(lst_idx_buy_primary)]) - 50: ])
				plt.axvline(x = int(lst_idx_buy_primary), linestyle = '-.')
				plt.plot(
							[
								int(signal_buy_primary['index_back'][lst_idx_buy_primary]) , 
								int(lst_idx_buy_primary)
							], 
							[
								dataset_5M_real[symbol]['low'][int(signal_buy_primary['index_back'][lst_idx_buy_primary])], 
								dataset_5M_real[symbol]['low'][int(lst_idx_buy_primary)]
							], 
							c = 'orange')
				...
				if lst_idx_buy_primary != 0:
					...
					if (res_pro_buy_primary.empty == False):
						...
						# signal = 'no_trade'	
					plt.axhline(resist_buy, c = 'black', linestyle = '-.')
					plt.axhline(protect_buy, c = 'black', linestyle = '-.')	
			...
			(len(dataset_5M_real[symbol]['close']) - 1 - lst_idx_buy_secondry) >= 5
			):
			...
			if (
				...
				plt.plot(dataset_5M_real[symbol]['low'][int(signal_buy_secondry['index_back'][int(lst_idx_buy_secondry)]) - 50: ])
				plt.axvline(x = int(lst_idx_buy_secondry), linestyle = '-.')
				plt.plot(
							[
								int(signal_buy_secondry['index_back'][lst_idx_buy_secondry]) , 
								int(lst_idx_buy_secondry)
							], 
							[
								dataset_5M_real[symbol]['low'][int(signal_buy_secondry['index_back'][lst_idx_buy_secondry])], 
								dataset_5M_real[symbol]['low'][int(lst_idx_buy_secondry)]
							], 
							c = 'orange')
			...
			(len(dataset_5M_real[symbol]['close']) - 1 - lst_idx_sell_primary) <= int(GL_Results_sell_primary['Divergence_num_exteremes_max'][0]/2) + 10 and
			(len(dataset_5M_real[symbol]['close']) - 1 - lst_idx_sell_primary) >= int(GL_Results_sell_primary['Divergence_num_exteremes_max'][0]/2) - 20
			):
			...
			if (
				...
				plt.plot(dataset_5M_real[symbol]['high'][int(signal_sell_primary['index_back'][int(lst_idx_sell_primary)]) - 50: ])
				plt.axvline(x = lst_idx_sell_primary, linestyle = '-.')

				plt.plot(
							[
								int(signal_sell_primary['index_back'][int(lst_idx_sell_primary)]) , 
								int(lst_idx_sell_primary)
							], 
							[
								dataset_5M_real[symbol]['high'][int(signal_sell_primary['index_back'][int(lst_idx_sell_primary)])], 
								dataset_5M_real[symbol]['high'][int(lst_idx_sell_primary)]
							], 
							c = 'orange')
				...
				if lst_idx_sell_primary != 0:
					...
					if dataset_5M_real[symbol]['high'][int(lst_idx_sell_primary)] < dataset_5M_real[symbol]['high'].iloc[-1]:
						...
						# candle_st = dataset_5M_real[symbol]['high'][int(lst_idx_sell_primary)] 
					else:
						candle_st = dataset_5M_real[symbol]['high'].iloc[-1] 
						# candle_st = dataset_5M_real[symbol]['high'][int(lst_idx_sell_primary)] 
					...
					plt.axhline(resist_sell, c = 'black', linestyle = '-.')
					plt.axhline(protect_sell, c = 'black', linestyle = '-.')
			...
			(len(dataset_5M_real[symbol]['close']) - 1 - lst_idx_sell_secondry) <= int(GL_Results_sell_secondry['Divergence_num_exteremes_max'][0]/2) and
			(len(dataset_5M_real[symbol]['close']) - 1 - lst_idx_sell_secondry) >= 1
			):

			#Price Action:
			price_action_flag = False
			price_action_flag_zone = False

			#Suplly Zone Finder: **********************************************************************

			if True:

				lst_idx_sell_secondry_1H = np.where(
													(dataset_1H_priceaction['time'].dt.year.to_numpy() == dataset_5M_real[symbol]['time'][int(lst_idx_sell_secondry)].year) &
													(dataset_1H_priceaction['time'].dt.month.to_numpy() == dataset_5M_real[symbol]['time'][int(lst_idx_sell_secondry)].month) &
													(dataset_1H_priceaction['time'].dt.day.to_numpy() == dataset_5M_real[symbol]['time'][int(lst_idx_sell_secondry)].day) &
													(dataset_1H_priceaction['time'].dt.hour.to_numpy() == dataset_5M_real[symbol]['time'][int(lst_idx_sell_secondry)].hour)
													)[0]

				top_points_1H_idx = np.where(
											(dataset_1H_priceaction['high'].to_numpy() >= dataset_5M_real[symbol]['high'].iloc[int(lst_idx_sell_secondry)]) &
											(dataset_1H_priceaction['open'].to_numpy() <= dataset_5M_real[symbol]['high'].iloc[int(lst_idx_sell_secondry)])
											)[0]

				supply_zone_upper = list([0,0])
				supply_zone_lowwer = list([0,0])
				supply_zone_counter = 0

				for pre_idx_supply_zone in top_points_1H_idx:

					for counter in range(-10, 10):

						if pre_idx_supply_zone + counter < 0: 
							counter += 1
							continue
						if pre_idx_supply_zone + counter + 1 >= len(dataset_1H_priceaction.index): break

						if (
							dataset_1H_priceaction['close'][pre_idx_supply_zone + counter] > dataset_1H_priceaction['open'][pre_idx_supply_zone + counter] and #Green Candle
							dataset_1H_priceaction['low'][pre_idx_supply_zone + counter] > dataset_1H_priceaction['close'][pre_idx_supply_zone + counter + 1] and #Engulfe
							dataset_1H_priceaction['open'][pre_idx_supply_zone + counter + 1] > dataset_1H_priceaction['close'][pre_idx_supply_zone + counter + 1]  #Red Candle
							):

							shadow_high = dataset_1H_priceaction['high'][pre_idx_supply_zone + counter + 1] - dataset_1H_priceaction['open'][pre_idx_supply_zone + counter + 1]
							shadow_low = dataset_1H_priceaction['close'][pre_idx_supply_zone + counter + 1] - dataset_1H_priceaction['low'][pre_idx_supply_zone + counter + 1]
							body_candle = dataset_1H_priceaction['open'][pre_idx_supply_zone + counter + 1] - dataset_1H_priceaction['close'][pre_idx_supply_zone + counter + 1]


							if body_candle > (shadow_low + shadow_high):

								supply_zone_upper.append(np.maximum(
																	dataset_1H_priceaction['high'][pre_idx_supply_zone + counter], 
																	dataset_1H_priceaction['high'][pre_idx_supply_zone + counter + 1]
																	)
														)

								supply_zone_lowwer.append(dataset_1H_priceaction['open'][pre_idx_supply_zone + counter])

								supply_zone_counter += 1


						elif pre_idx_supply_zone + counter + 2 >= len(dataset_1H_priceaction.index): break

						elif (
							dataset_1H_priceaction['close'][pre_idx_supply_zone + counter] > dataset_1H_priceaction['open'][pre_idx_supply_zone + counter] and #Green Candle
							dataset_1H_priceaction['low'][pre_idx_supply_zone + counter] > dataset_1H_priceaction['close'][pre_idx_supply_zone + counter + 2] and #Engulfe
							dataset_1H_priceaction['open'][pre_idx_supply_zone + counter + 1] > dataset_1H_priceaction['close'][pre_idx_supply_zone + counter + 1] and  #Red Candle
							dataset_1H_priceaction['open'][pre_idx_supply_zone + counter + 2] > dataset_1H_priceaction['close'][pre_idx_supply_zone + counter + 2] #Red Candle
							):

							shadow_high = dataset_1H_priceaction['high'][pre_idx_supply_zone + counter + 2] - dataset_1H_priceaction['open'][pre_idx_supply_zone + counter + 2]
							shadow_low = dataset_1H_priceaction['close'][pre_idx_supply_zone + counter + 2] - dataset_1H_priceaction['low'][pre_idx_supply_zone + counter + 2]
							body_candle = dataset_1H_priceaction['open'][pre_idx_supply_zone + counter + 2] - dataset_1H_priceaction['close'][pre_idx_supply_zone + counter + 2]


							if body_candle > (shadow_low + shadow_high):

								supply_zone_upper.append(np.max(
																[
																dataset_1H_priceaction['high'][pre_idx_supply_zone + counter],
																dataset_1H_priceaction['high'][pre_idx_supply_zone + counter + 1],
																dataset_1H_priceaction['high'][pre_idx_supply_zone + counter + 2]
																]
																)
														)
								supply_zone_lowwer.append(dataset_1H_priceaction['open'][pre_idx_supply_zone + counter])
				...
				supply_zone_lowwer_1H = pd.DataFrame(supply_zone_lowwer, columns = ['supply_lowwer'])
				supply_zone_upper_1H = pd.DataFrame(supply_zone_upper, columns = ['supply_upper'])

				for deleter_high in range((lst_idx_sell_secondry - int(GL_Results_sell_secondry['Divergence_num_exteremes_max'][0]/2)), lst_idx_sell_secondry-4):

					delete_index = supply_zone_lowwer_1H.index[(dataset_5M_real[symbol]['high'].iloc[deleter_high] >= supply_zone_lowwer_1H['supply_lowwer'])]
					supply_zone_lowwer_1H = supply_zone_lowwer_1H.drop(index = delete_index)
					supply_zone_upper_1H = supply_zone_upper_1H.drop(index = delete_index)

				supply_zone_lowwer_index = supply_zone_lowwer_1H.index[(supply_zone_lowwer_1H['supply_lowwer'] <= dataset_5M_real[symbol]['high'].iloc[lst_idx_sell_secondry])]
				dist_supply_zone_upper = supply_zone_upper_1H.copy(deep = True)
				dist_supply_zone_upper['supply_upper'] = supply_zone_upper_1H['supply_upper'] - (dataset_1H_priceaction['high'].iloc[int(lst_idx_sell_secondry_1H)])
				
				dist_supply_zone_upper = dist_supply_zone_upper.drop(index = dist_supply_zone_upper.index[dist_supply_zone_upper['supply_upper'] <= 0])
				supply_zone_upper_1H = supply_zone_upper_1H['supply_upper'][dist_supply_zone_upper['supply_upper'].idxmin()]
				# supply_zone_lowwer_1H = supply_zone_lowwer_1H['supply_lowwer'][supply_zone_lowwer_index].max()
				supply_zone_lowwer_1H = supply_zone_lowwer_1H['supply_lowwer'][dist_supply_zone_upper['supply_upper'].idxmin()]

				diff_percent_supply_zone = ((supply_zone_upper_1H - supply_zone_lowwer_1H)/supply_zone_lowwer_1H) * 100

				if True:#diff_percent_supply_zone > pr_parameters_sell_secondry.elements['st_percent_max']:

					top_points_5M_idx = np.where(
												(dataset_5M_real[symbol]['high'][:lst_idx_sell_secondry+1].to_numpy() <= supply_zone_upper_1H) &
												(dataset_5M_real[symbol]['high'][:lst_idx_sell_secondry+1].to_numpy() >= supply_zone_lowwer_1H)
												)[0]
					...
					for pre_idx_supply_zone in top_points_5M_idx:
						...
						for counter in range(-10, 10):
							...
							if pre_idx_supply_zone + counter < 0: 
								counter += 1
								continue
							if pre_idx_supply_zone + counter + 1 >= len(dataset_5M_real[symbol].index): break

							if (
								dataset_5M_real[symbol]['close'][pre_idx_supply_zone + counter] > dataset_5M_real[symbol]['open'][pre_idx_supply_zone + counter] and #Green Candle
								dataset_5M_real[symbol]['low'][pre_idx_supply_zone + counter] > dataset_5M_real[symbol]['close'][pre_idx_supply_zone + counter + 1] and #Engulfe
								dataset_5M_real[symbol]['open'][pre_idx_supply_zone + counter + 1] > dataset_5M_real[symbol]['close'][pre_idx_supply_zone + counter + 1]  #Red Candle
								):

								shadow_high = dataset_5M_real[symbol]['high'][pre_idx_supply_zone + counter + 1] - dataset_5M_real[symbol]['open'][pre_idx_supply_zone + counter + 1]
								shadow_low = dataset_5M_real[symbol]['close'][pre_idx_supply_zone + counter + 1] - dataset_5M_real[symbol]['low'][pre_idx_supply_zone + counter + 1]
								body_candle = dataset_5M_real[symbol]['open'][pre_idx_supply_zone + counter + 1] - dataset_5M_real[symbol]['close'][pre_idx_supply_zone + counter + 1]

								if body_candle > (shadow_low + shadow_high):

									supply_zone_upper.append(np.maximum(
																		dataset_5M_real[symbol]['high'][pre_idx_supply_zone + counter], 
																		dataset_5M_real[symbol]['high'][pre_idx_supply_zone + counter + 1]
																		)
															)

									supply_zone_lowwer.append(dataset_5M_real[symbol]['open'][pre_idx_supply_zone + counter])

									supply_zone_counter += 1


							elif pre_idx_supply_zone + counter + 2 >= len(dataset_5M_real[symbol].index): break

							elif (
								dataset_5M_real[symbol]['close'][pre_idx_supply_zone + counter] > dataset_5M_real[symbol]['open'][pre_idx_supply_zone + counter] and #Green Candle
								dataset_5M_real[symbol]['low'][pre_idx_supply_zone + counter] > dataset_5M_real[symbol]['close'][pre_idx_supply_zone + counter + 2] and #Engulfe
								dataset_5M_real[symbol]['open'][pre_idx_supply_zone + counter + 1] > dataset_5M_real[symbol]['close'][pre_idx_supply_zone + counter + 1] and  #Red Candle
								dataset_5M_real[symbol]['open'][pre_idx_supply_zone + counter + 2] > dataset_5M_real[symbol]['close'][pre_idx_supply_zone + counter + 2] #Red Candle
								):

								shadow_high = dataset_5M_real[symbol]['high'][pre_idx_supply_zone + counter + 2] - dataset_5M_real[symbol]['open'][pre_idx_supply_zone + counter + 2]
								shadow_low = dataset_5M_real[symbol]['close'][pre_idx_supply_zone + counter + 2] - dataset_5M_real[symbol]['low'][pre_idx_supply_zone + counter + 2]
								body_candle = dataset_5M_real[symbol]['open'][pre_idx_supply_zone + counter + 2] - dataset_5M_real[symbol]['close'][pre_idx_supply_zone + counter + 2]
								...
								if body_candle > (shadow_low + shadow_high):
									...
									supply_zone_upper.append(np.max(
																	...
																	dataset_5M_real[symbol]['high'][pre_idx_supply_zone + counter],
																	dataset_5M_real[symbol]['high'][pre_idx_supply_zone + counter + 1],
																	dataset_5M_real[symbol]['high'][pre_idx_supply_zone + counter + 2]
																	]
																	)
															)
									supply_zone_lowwer.append(dataset_5M_real[symbol]['open'][pre_idx_supply_zone + counter])
					...
					supply_zone_lowwer = pd.DataFrame(supply_zone_lowwer, columns = ['supply_lowwer'])
					supply_zone_upper = pd.DataFrame(supply_zone_upper, columns = ['supply_upper'])

					idx_min_low = dataset_5M_real[symbol]['low'][int(signal_sell_secondry['index_back'][int(lst_idx_sell_secondry)]):int(lst_idx_sell_secondry)].idxmin()

					for deleter_high in range(int(idx_min_low), int(lst_idx_sell_secondry)):

						delete_index = supply_zone_upper.index[(dataset_5M_real[symbol]['high'].iloc[deleter_high] > supply_zone_upper['supply_upper'])]
						supply_zone_lowwer = supply_zone_lowwer.drop(index = delete_index)
						supply_zone_upper = supply_zone_upper.drop(index = delete_index)


					# with pd.option_context('display.max_rows', None, 'display.max_columns', None):
					# 	print(supply_zone_lowwer_1H['supply_lowwer'])

					if supply_zone_upper.empty == False:
						supply_zone_lowwer_index = supply_zone_lowwer.index[(supply_zone_lowwer['supply_lowwer'] <= dataset_5M_real[symbol]['high'].iloc[lst_idx_sell_secondry])]
						dist_supply_zone_upper = supply_zone_upper.copy(deep = True)
						dist_supply_zone_upper['supply_upper'] = supply_zone_upper['supply_upper'] - (dataset_5M_real[symbol]['high'].iloc[lst_idx_sell_secondry])
						
						dist_supply_zone_upper = dist_supply_zone_upper.drop(index = dist_supply_zone_upper.index[dist_supply_zone_upper['supply_upper'] <= 0])
						supply_zone_upper = supply_zone_upper['supply_upper'][dist_supply_zone_upper['supply_upper'].idxmin()]
						supply_zone_lowwer = supply_zone_lowwer['supply_lowwer'][supply_zone_lowwer_index].max()
						# supply_zone_lowwer = supply_zone_lowwer['supply_lowwer'][dist_supply_zone_upper['supply_upper'].idxmin()]
					else:
						supply_zone_upper = 0
						supply_zone_lowwer = 0

				else:
					supply_zone_upper = supply_zone_upper_1H
					supply_zone_lowwer = supply_zone_lowwer_1H
			...
			if True:
				...
				down_points_1H_idx = np.where(
											(dataset_1H_priceaction['open'] <= dataset_1H_priceaction['low'].iloc[-1])
											)[0]

				demand_zone_upper = list([dataset_1H_priceaction['low'].iloc[-1],dataset_1H_priceaction['low'].iloc[-1]])
				demand_zone_lowwer = list([dataset_1H_priceaction['low'].iloc[-1],dataset_1H_priceaction['low'].iloc[-1]])
				demand_zone_counter = 0

				for pre_idx_demand_zone in down_points_1H_idx:

					for counter in range(-10, 10):

						if pre_idx_demand_zone + counter < 0: 
							counter += 1
							continue
						if pre_idx_demand_zone + counter + 1 >= len(dataset_1H_priceaction.index): break

						if (
							dataset_1H_priceaction['close'][pre_idx_demand_zone + counter] < dataset_1H_priceaction['open'][pre_idx_demand_zone + counter] and #Red Candle
							dataset_1H_priceaction['high'][pre_idx_demand_zone + counter] <= dataset_1H_priceaction['close'][pre_idx_demand_zone + counter + 1] and #Engulfe
							dataset_1H_priceaction['open'][pre_idx_demand_zone + counter + 1] < dataset_1H_priceaction['close'][pre_idx_demand_zone + counter + 1]  #Green Candle
							):

							shadow_high = dataset_1H_priceaction['high'][pre_idx_demand_zone + counter + 1] - dataset_1H_priceaction['close'][pre_idx_demand_zone + counter + 1]
							shadow_low = dataset_1H_priceaction['open'][pre_idx_demand_zone + counter + 1] - dataset_1H_priceaction['low'][pre_idx_demand_zone + counter + 1]
							body_candle = dataset_1H_priceaction['close'][pre_idx_demand_zone + counter + 1] - dataset_1H_priceaction['open'][pre_idx_demand_zone + counter + 1]

							if body_candle > (shadow_low + shadow_high):

								demand_zone_lowwer.append(np.minimum(
																	dataset_1H_priceaction['low'][pre_idx_demand_zone + counter], 
																	dataset_1H_priceaction['low'][pre_idx_demand_zone + counter + 1]
																	)
														)

								demand_zone_upper.append(dataset_1H_priceaction['open'][pre_idx_demand_zone + counter])

								demand_zone_counter += 1

								# print('Demand up = ', demand_zone_upper, 'Demand low = ', demand_zone_lowwer)


						elif pre_idx_demand_zone + counter + 2 >= len(dataset_1H_priceaction.index): break

						elif (
							dataset_1H_priceaction['close'][pre_idx_demand_zone + counter] < dataset_1H_priceaction['open'][pre_idx_demand_zone + counter] and #Red Candle
							dataset_1H_priceaction['high'][pre_idx_demand_zone + counter] <= dataset_1H_priceaction['close'][pre_idx_demand_zone + counter + 2] and #Engulfe
							dataset_1H_priceaction['open'][pre_idx_demand_zone + counter + 1] < dataset_1H_priceaction['close'][pre_idx_demand_zone + counter + 1] and  #Green Candle
							dataset_1H_priceaction['open'][pre_idx_demand_zone + counter + 2] < dataset_1H_priceaction['close'][pre_idx_demand_zone + counter + 2] #Green Candle
							):

							shadow_high = dataset_1H_priceaction['high'][pre_idx_demand_zone + counter + 2] - dataset_1H_priceaction['close'][pre_idx_demand_zone + counter + 2]
							shadow_low = dataset_1H_priceaction['open'][pre_idx_demand_zone + counter + 2] - dataset_1H_priceaction['low'][pre_idx_demand_zone + counter + 2]
							body_candle = dataset_1H_priceaction['close'][pre_idx_demand_zone + counter + 2] - dataset_1H_priceaction['open'][pre_idx_demand_zone + counter + 2]

							if body_candle > (shadow_low + shadow_high):

								demand_zone_lowwer.append(np.min(
															[
															dataset_1H_priceaction['low'][pre_idx_demand_zone + counter],
															dataset_1H_priceaction['low'][pre_idx_demand_zone + counter + 1],
															dataset_1H_priceaction['low'][pre_idx_demand_zone + counter + 2]
															]
															)
														)
				...
				try:
					demand_zone_upper_1H = pd.DataFrame(demand_zone_upper, columns = ['demand_upper'])
					dist_demand_zone_upper = demand_zone_upper_1H - (dataset_1H_priceaction['low'][-4:-1].mean() * 0.9996)
					dist_demand_zone_upper = dist_demand_zone_upper.drop(index = dist_demand_zone_upper.index[dist_demand_zone_upper['demand_upper'] >= 0])
					dist_demand_zone_upper = dist_demand_zone_upper.drop_duplicates(subset = ['demand_upper'], keep='last', inplace=False, ignore_index=False)

					demand_zone_upper_1H = demand_zone_upper_1H['demand_upper'][dist_demand_zone_upper['demand_upper'].idxmax()]

					

					demand_zone_lowwer_1H = pd.DataFrame(demand_zone_lowwer, columns = ['demand_lowwer'])
					dist_demand_zone_lowwer = demand_zone_lowwer_1H - (dataset_1H_priceaction['low'][-4:-1].mean() * 0.9996)
					dist_demand_zone_lowwer = dist_demand_zone_lowwer.drop(index = dist_demand_zone_lowwer.index[dist_demand_zone_lowwer['demand_lowwer'] >= 0])
					dist_demand_zone_lowwer = dist_demand_zone_lowwer.drop_duplicates(subset = ['demand_lowwer'], keep='last', inplace=False, ignore_index=False)
					demand_zone_lowwer_1H = demand_zone_lowwer_1H['demand_lowwer'][dist_demand_zone_lowwer['demand_lowwer'].idxmax()]

					demand_zone_upper = demand_zone_upper_1H
					demand_zone_lowwer = demand_zone_lowwer_1H
				except Exception as ex:
					# print('Demand 1H: ', ex)
					demand_zone_lowwer_1H = 0
					demand_zone_upper_1H = 0

					demand_zone_upper = 0
					demand_zone_lowwer = 0

				if True:#((dataset_5M_real[symbol]['low'].iloc[-1] - demand_zone_upper_1H)/demand_zone_upper_1H) * 100 > GL_Results_sell_secondry['tp_percent_max'][0]:
					down_points_1H_idx = np.where(
												(dataset_5M_real[symbol]['open'][:-1] <= dataset_5M_real[symbol]['low'].iloc[-1]) & 
												(dataset_5M_real[symbol]['open'][:-1] >= demand_zone_upper_1H)
												)[0]

					# print(down_points_1H_idx)

					demand_zone_upper = list([ dataset_5M_real[symbol]['low'].iloc[-1], dataset_5M_real[symbol]['low'].iloc[-1]])
					demand_zone_lowwer = list([ dataset_5M_real[symbol]['low'].iloc[-1], dataset_5M_real[symbol]['low'].iloc[-1]])
					demand_zone_counter = 0
					...
					for pre_idx_demand_zone in down_points_1H_idx:
						...
						for counter in range(-10, 10):
							...
							if pre_idx_demand_zone + counter < 0: 
								counter += 1
								continue
							if pre_idx_demand_zone + counter + 1 >= len(dataset_5M_real[symbol].index): break

							if (
								dataset_5M_real[symbol]['close'][pre_idx_demand_zone + counter] < dataset_5M_real[symbol]['open'][pre_idx_demand_zone + counter] and #Red Candle
								dataset_5M_real[symbol]['high'][pre_idx_demand_zone + counter] <= dataset_5M_real[symbol]['close'][pre_idx_demand_zone + counter + 1] and #Engulfe
								dataset_5M_real[symbol]['open'][pre_idx_demand_zone + counter + 1] < dataset_5M_real[symbol]['close'][pre_idx_demand_zone + counter + 1]  #Green Candle
								):

								shadow_high = dataset_5M_real[symbol]['high'][pre_idx_demand_zone + counter + 1] - dataset_5M_real[symbol]['close'][pre_idx_demand_zone + counter + 1]
								shadow_low = dataset_5M_real[symbol]['open'][pre_idx_demand_zone + counter + 1] - dataset_5M_real[symbol]['low'][pre_idx_demand_zone + counter + 1]
								body_candle = dataset_5M_real[symbol]['close'][pre_idx_demand_zone + counter + 1] - dataset_5M_real[symbol]['open'][pre_idx_demand_zone + counter + 1]

								if body_candle > (shadow_low + shadow_high):

									demand_zone_lowwer.append(np.minimum(
																		dataset_5M_real[symbol]['low'][pre_idx_demand_zone + counter], 
																		dataset_5M_real[symbol]['low'][pre_idx_demand_zone + counter + 1]
																		)
															)

									demand_zone_upper.append(dataset_5M_real[symbol]['open'][pre_idx_demand_zone + counter])
							...
							elif pre_idx_demand_zone + counter + 2 >= len(dataset_5M_real[symbol].index): break

							elif (
								dataset_5M_real[symbol]['close'][pre_idx_demand_zone + counter] < dataset_5M_real[symbol]['open'][pre_idx_demand_zone + counter] and #Red Candle
								dataset_5M_real[symbol]['high'][pre_idx_demand_zone + counter] <= dataset_5M_real[symbol]['close'][pre_idx_demand_zone + counter + 2] and #Engulfe
								dataset_5M_real[symbol]['open'][pre_idx_demand_zone + counter + 1] < dataset_5M_real[symbol]['close'][pre_idx_demand_zone + counter + 1] and  #Green Candle
								dataset_5M_real[symbol]['open'][pre_idx_demand_zone + counter + 2] < dataset_5M_real[symbol]['close'][pre_idx_demand_zone + counter + 2] #Green Candle
								):

								shadow_high = dataset_5M_real[symbol]['high'][pre_idx_demand_zone + counter + 2] - dataset_5M_real[symbol]['close'][pre_idx_demand_zone + counter + 2]
								shadow_low = dataset_5M_real[symbol]['open'][pre_idx_demand_zone + counter + 2] - dataset_5M_real[symbol]['low'][pre_idx_demand_zone + counter + 2]
								body_candle = dataset_5M_real[symbol]['close'][pre_idx_demand_zone + counter + 2] - dataset_5M_real[symbol]['open'][pre_idx_demand_zone + counter + 2]
								...
								if body_candle > (shadow_low + shadow_high):
									...
									demand_zone_lowwer.append(np.min(
																...
																dataset_5M_real[symbol]['low'][pre_idx_demand_zone + counter],
																dataset_5M_real[symbol]['low'][pre_idx_demand_zone + counter + 1],
																dataset_5M_real[symbol]['low'][pre_idx_demand_zone + counter + 2]
																]
																)
															)
					...
					try:
						demand_zone_upper_5M = pd.DataFrame(demand_zone_upper, columns = ['demand_upper'])
						dist_demand_zone_upper = demand_zone_upper_5M - (dataset_5M_real[symbol]['low'].iloc[-1] * 0.9996)
						dist_demand_zone_upper = dist_demand_zone_upper.drop(index = dist_demand_zone_upper.index[dist_demand_zone_upper['demand_upper'] >= 0])
						dist_demand_zone_upper = dist_demand_zone_upper.drop_duplicates(subset = ['demand_upper'], keep='last', inplace=False, ignore_index=False)

						demand_zone_upper_5M = demand_zone_upper_5M['demand_upper'][dist_demand_zone_upper['demand_upper'].idxmax()]

						

						demand_zone_lowwer_5M = pd.DataFrame(demand_zone_lowwer, columns = ['demand_lowwer'])
						dist_demand_zone_lowwer = demand_zone_lowwer_5M - (dataset_5M_real[symbol]['low'].iloc[-1] * 0.9996)
						dist_demand_zone_lowwer = dist_demand_zone_lowwer.drop(index = dist_demand_zone_lowwer.index[dist_demand_zone_lowwer['demand_lowwer'] >= 0])
						dist_demand_zone_lowwer = dist_demand_zone_lowwer.drop_duplicates(subset = ['demand_lowwer'], keep='last', inplace=False, ignore_index=False)
						demand_zone_lowwer_5M = demand_zone_lowwer_5M['demand_lowwer'][dist_demand_zone_lowwer['demand_lowwer'].idxmax()]

						demand_zone_upper = demand_zone_upper_5M
						demand_zone_lowwer = demand_zone_lowwer_5M

					except Exception as ex:
						# print('Demand 5M: ', ex)
						# print('up = ', demand_zone_upper)
						# print('low = ', demand_zone_lowwer)
						demand_zone_upper = demand_zone_upper_1H
						demand_zone_lowwer = demand_zone_lowwer_1H
			...
			if (
				dataset_1H_priceaction['close'].iloc[-3] > dataset_1H_priceaction['open'].iloc[-3] and #Green Candle 1H
				dataset_1H_priceaction['open'].iloc[-2] > dataset_1H_priceaction['close'].iloc[-2] and #Red Candle 1H
				True
				):
				flag_priceaction_1H = True

			elif (
				dataset_1H_priceaction['close'].iloc[-4] > dataset_1H_priceaction['open'].iloc[-4] and #Green Candle 1H
				dataset_1H_priceaction['open'].iloc[-2] > dataset_1H_priceaction['close'].iloc[-2] and #Red Candle 1H
				dataset_1H_priceaction['open'].iloc[-3] > dataset_1H_priceaction['close'].iloc[-3] and #Red Candle 1H
				True
				):
				flag_priceaction_1H = True
			...
			shadow_high_1 = dataset_5M_real[symbol]['high'].iloc[-2] - dataset_5M_real[symbol]['open'].iloc[-2]
			shadow_low_1 = dataset_5M_real[symbol]['close'].iloc[-2] - dataset_5M_real[symbol]['low'].iloc[-2]
			body_candle_1 = dataset_5M_real[symbol]['open'].iloc[-2] - dataset_5M_real[symbol]['close'].iloc[-2]

			if (
				(
					supply_zone_lowwer <= dataset_5M_real[symbol]['high'].iloc[-3] <= supply_zone_upper 
				) and
				(
					supply_zone_lowwer <= dataset_5M_real[symbol]['high'].iloc[-2] <= supply_zone_upper 
				) and
				dataset_5M_real[symbol]['close'].iloc[-3] > dataset_5M_real[symbol]['open'].iloc[-3] and #Green Candle
				dataset_5M_real[symbol]['open'].iloc[-3] >= dataset_5M_real[symbol]['low'].iloc[-2] and #Engulfe
				dataset_5M_real[symbol]['open'].iloc[-2] > dataset_5M_real[symbol]['close'].iloc[-2] and #Red Candle
				dataset_5M_real[symbol]['low'].iloc[-3] <= supply_zone_lowwer and
				dataset_5M_real[symbol]['low'].iloc[-2] <= supply_zone_lowwer and
				(body_candle_1) > (shadow_high_1 + shadow_low_1)/2 and
				supply_zone_lowwer != 0 and
				supply_zone_upper != 0 and
				flag_priceaction_1H == True and
				True
				):
				# if supply_zone_lowwer <= dataset_5M_real[symbol]['high'].iloc[-1] <= supply_zone_upper:
				price_action_flag = True
				candle_st = supply_zone_upper
				candle_tp = dataset_5M_real[symbol]['low'].iloc[-1]
				candle_pr_index = dataset_5M_real[symbol].index[-3]

			shadow_high_1 = dataset_5M_real[symbol]['high'].iloc[-2] - dataset_5M_real[symbol]['open'].iloc[-2]
			shadow_low_1 = dataset_5M_real[symbol]['close'].iloc[-2] - dataset_5M_real[symbol]['low'].iloc[-2]
			body_candle_1 = dataset_5M_real[symbol]['open'].iloc[-2] - dataset_5M_real[symbol]['close'].iloc[-2]

			shadow_high_2 = dataset_5M_real[symbol]['high'].iloc[-3] - dataset_5M_real[symbol]['open'].iloc[-3]
			shadow_low_2 = dataset_5M_real[symbol]['close'].iloc[-3] - dataset_5M_real[symbol]['low'].iloc[-3]
			body_candle_2 = dataset_5M_real[symbol]['open'].iloc[-3] - dataset_5M_real[symbol]['close'].iloc[-3]

			if (
				supply_zone_lowwer <= dataset_5M_real[symbol]['high'].iloc[-4] <= supply_zone_upper and
				supply_zone_lowwer <= dataset_5M_real[symbol]['high'].iloc[-3] <= supply_zone_upper and
				dataset_5M_real[symbol]['low'].iloc[-4] <= supply_zone_lowwer and
				dataset_5M_real[symbol]['low'].iloc[-2] <= supply_zone_lowwer and
				dataset_5M_real[symbol]['close'].iloc[-4] > dataset_5M_real[symbol]['open'].iloc[-4] and #Green Candle
				dataset_5M_real[symbol]['open'].iloc[-4] >= dataset_5M_real[symbol]['low'].iloc[-2] and #Engulfe
				dataset_5M_real[symbol]['open'].iloc[-2] > dataset_5M_real[symbol]['close'].iloc[-2] and #Red Candle
				dataset_5M_real[symbol]['open'].iloc[-3] > dataset_5M_real[symbol]['close'].iloc[-3] and #Red Candle
				(
					(body_candle_1) > (shadow_high_1 + shadow_low_1)/2 or
					(body_candle_2) > (shadow_high_2 + shadow_low_2)/2
					) and
				supply_zone_lowwer != 0 and
				supply_zone_upper != 0 and
				flag_priceaction_1H == True and
				...
				candle_st = supply_zone_upper
				candle_tp = dataset_5M_real[symbol]['low'].iloc[-1]
				candle_pr_index = dataset_5M_real[symbol].index[-4]
			...
			if (
				(abs(dataset_5M_real[symbol]['high'].iloc[-3] - supply_zone_upper)/supply_zone_upper) <= 0.0004 and
				(abs(dataset_5M_real[symbol]['high'].iloc[-2] - supply_zone_upper)/supply_zone_upper) <= 0.0004 and
				dataset_5M_real[symbol]['high'].iloc[-3] >= supply_zone_upper and
				dataset_5M_real[symbol]['close'].iloc[-3] > dataset_5M_real[symbol]['open'].iloc[-3] and #Green Candle
				dataset_5M_real[symbol]['open'].iloc[-3] >= dataset_5M_real[symbol]['low'].iloc[-2] and #Engulfe
				dataset_5M_real[symbol]['open'].iloc[-2] > dataset_5M_real[symbol]['close'].iloc[-2] and #Red Candle
				dataset_5M_real[symbol]['low'].iloc[-3] <= supply_zone_lowwer and
			...
			if (
				(abs(dataset_5M_real[symbol]['high'].iloc[-4] - supply_zone_upper)/supply_zone_upper) <= 0.0004 and
				(abs(dataset_5M_real[symbol]['high'].iloc[-3] - supply_zone_upper)/supply_zone_upper) <= 0.0004 and
				dataset_5M_real[symbol]['high'].iloc[-4] >= supply_zone_upper and
				dataset_5M_real[symbol]['close'].iloc[-4] > dataset_5M_real[symbol]['open'].iloc[-4] and #Green Candle
				dataset_5M_real[symbol]['open'].iloc[-4] >= dataset_5M_real[symbol]['low'].iloc[-2] and #Engulfe
				dataset_5M_real[symbol]['open'].iloc[-2] > dataset_5M_real[symbol]['close'].iloc[-2] and #Red Candle
				dataset_5M_real[symbol]['open'].iloc[-3] > dataset_5M_real[symbol]['close'].iloc[-3] and #Red Candle
			...
			try:
				res_pro_sell_secondry = self.ProfitFinder(
														...
														dataset_1H = dataset_1H,
														symbol = symbol,
														index_candle = int(lst_idx_sell_secondry), 
				...
				if res_pro_sell_secondry['high_upper'][int(lst_idx_sell_secondry)] != 0:
					resist_sell = res_pro_sell_secondry['high_upper'][int(lst_idx_sell_secondry)]
					tokhmi_upper = False
				else:
					resist_sell = dataset_5M_real[symbol]['high'].iloc[-1] * 1.001
					tokhmi_upper = True

				if res_pro_sell_secondry['high_lower'][int(lst_idx_sell_secondry)] != 0:
					protect_sell = res_pro_sell_secondry['high_lower'][int(lst_idx_sell_secondry)]
					tokhmi_lowwer = False
				else:
					protect_sell = dataset_5M_real[symbol]['low'].iloc[-1] * 1.001
					tokhmi_lowwer = True
			...
			if supply_zone_upper == 0:
				supply_zone_upper = dataset_5M_real[symbol]['high'].iloc[-1] * 1.001
				tokhmi_zone_up = True
			else:
				tokhmi_zone_up = False

			if supply_zone_lowwer == 0:
				supply_zone_lowwer = dataset_5M_real[symbol]['high'].iloc[-1] * 1.001
				tokhmi_zone_low = True
			else:
				tokhmi_zone_low = False

			signal = 'sell_secondry'
			# if price_action_flag:
			print(
				'index: ', candle_index, 
				' flag: ', price_action_flag, 
				' Tokhmi High: ', tokhmi_upper,
				' Tokhmi Low: ', tokhmi_lowwer,
				' Tokhmi Zone Up: ', tokhmi_zone_up,
				' Tokhmi Zone Low: ', tokhmi_zone_low,
				)
			# if signal_sell_secondry['indicator_front'].iloc[-1] > GL_Results_sell_secondry['value_front_intervals_pr_lower'][0]: 
			# 	price_action_flag = True
			...
			if (
				...
				# macd_calc_sell_secondry[GL_Results_sell_secondry['MACD_column_div'][0]][lst_idx_sell_secondry] > macd_calc_sell_secondry[GL_Results_sell_secondry['MACD_column_div'][0]][lst_idx_sell_secondry - 1] and
				# macd_calc_sell_secondry[GL_Results_sell_secondry['MACD_column_div'][0]][lst_idx_sell_secondry] > macd_calc_sell_secondry[GL_Results_sell_secondry['MACD_column_div'][0]][lst_idx_sell_secondry - 2] and
				dataset_5M_real[symbol]['high'][signal_sell_secondry['index_back'][lst_idx_sell_secondry]] > dataset_5M_real[symbol]['high'][lst_idx_sell_secondry] and
				# signal_sell_secondry['indicator_back'][int(lst_idx_sell_secondry)] >= GL_Results_sell_secondry['value_back_intervals_pr_lower'][0]/10 and
				price_action_flag == True
				# dataset_5M_real[symbol]['high'].iloc[-1] * 0.999 >= max_zigzag
				...
				print('Front = ', signal_sell_secondry['indicator_front'][int(lst_idx_sell_secondry)])
				print('Back = ', signal_sell_secondry['indicator_back'][int(lst_idx_sell_secondry)])


				# plt.plot(zigzag_plot, c = 'r')
				

				# plt.scatter(down_points.index, down_points, c = 'r')
				# plt.scatter(top_points.index, top_points, c = 'g')

				# min_zigzag = down_points.values[down_points.index >= int(signal_sell_secondry['index_back'][lst_idx_sell_secondry]) + (candle_index - len(dataset_5M_real[symbol].index))].min()
				# idx_min_zigzag = down_points[down_points.index >= int(signal_sell_secondry['index_back'][lst_idx_sell_secondry]) + (candle_index - len(dataset_5M_real[symbol].index))].idxmin()

				# plt.scatter(idx_max_zigzag, max_zigzag, c = 'black')
				# plt.scatter(idx_min_zigzag, min_zigzag, c = 'purple')


				# # plt.plot(dataset_5M_real[symbol]['high'][int(signal_sell_secondry['index_back'][int(lst_idx_sell_secondry)]): ])
				# plt.axvline(x = int(lst_idx_sell_secondry) + (candle_index - len(dataset_5M_real[symbol].index)), linestyle = 'dotted', c = 'purple')
				# plt.plot(
				# 			[
				# 				int(signal_sell_secondry['index_back'][lst_idx_sell_secondry]) + (candle_index - len(dataset_5M_real[symbol].index)) , 
				# 				int(lst_idx_sell_secondry) + (candle_index - len(dataset_5M_real[symbol].index))
				# 			], 
				# 			[
				# 				dataset_5M_real[symbol]['high'][int(signal_sell_secondry['index_back'][lst_idx_sell_secondry])], 
				# 				dataset_5M_real[symbol]['high'][int(lst_idx_sell_secondry)]
				# 			], 
				# 			c = 'orange')
				...
				if lst_idx_sell_secondry != 0:
					...
					try:
						res_pro_sell_secondry = self.ProfitFinder(
																...
																dataset_1H = dataset_1H,
																symbol = symbol,
																index_candle = candle_pr_index, 
																signal = signal_sell_secondry,
																sigtype = 'sell', 
																pr_parameters = pr_parameters_sell_secondry, 
																pr_config = pr_config_sell_secondry
																)
						# lst_idx_sell_secondry = candle_pr_index
						res_pro_sell_secondry['high_upper'][int(lst_idx_sell_secondry)] = np.max([supply_zone_upper_1H , res_pro_sell_secondry['high_upper'][int(lst_idx_sell_secondry)]])

						if supply_zone_lowwer != 0:
							res_pro_sell_secondry['low_lower'][int(lst_idx_sell_secondry)] = np.min([demand_zone_upper , res_pro_sell_secondry['low_lower'][int(lst_idx_sell_secondry)]])
					except Exception as ex:
						# print('ERROR PR Last Signal: ',ex)
						res_pro_sell_secondry = pd.DataFrame(np.zeros(int(lst_idx_sell_secondry) + 1))
						res_pro_sell_secondry['high_upper'] = np.nan
						...
						if supply_zone_upper_1H != 0:
							res_pro_sell_secondry['high_upper'][int(lst_idx_sell_secondry)] = supply_zone_upper_1H
						
						if demand_zone_upper != 0:
							res_pro_sell_secondry['low_lower'][int(lst_idx_sell_secondry)] = np.max([demand_zone_upper, demand_zone_lowwer])
					...
					if (res_pro_sell_secondry.empty == False):
						...
						if diff_pr_top_sell_secondry < pr_parameters_sell_secondry.elements['st_percent_min']:
						...
						if diff_pr_down_sell_secondry < pr_parameters_sell_secondry.elements['tp_percent_min']:
						...
						if diff_pr_down_sell_secondry < diff_pr_top_sell_secondry:
							diff_pr_down_sell_secondry = diff_pr_top_sell_secondry
							res_pro_sell_secondry['low_lower'][int(lst_idx_sell_secondry)] = candle_tp * (1-(diff_pr_down_sell_secondry/100))
						...
						resist_sell = (res_pro_sell_secondry['high_upper'][int(lst_idx_sell_secondry)]) * (1.0004)
						protect_sell = (res_pro_sell_secondry['low_lower'][int(lst_idx_sell_secondry)]) * (1.0004)
						
						signal = 'sell_secondry'

						if dataset_5M_real[symbol]['high'].iloc[-1] * (1.0004) >= resist_sell: 
							resist_sell = dataset_5M_real[symbol]['high'].iloc[-1] * (1 + (pr_parameters_sell_secondry.elements['st_percent_max']/100))

						if dataset_5M_real[symbol]['low'].iloc[-1] <= protect_sell: 
							protect_sell = dataset_5M_real[symbol]['low'].iloc[-1] * (1 + (pr_parameters_sell_secondry.elements['tp_percent_max']/100))
						...
						# signal = 'no_trade'	
					# plt.axhline(resist_sell, c = 'purple', linestyle = '-.')
					# plt.axhline(protect_sell, c = 'black', linestyle = '-.')	
				...
				# signal = 'no_trade'	
				#Felan Comment Kardam Baraye Test
		...
		if (
			...
			return signal, protect_sell, resist_sell, supply_zone_upper, supply_zone_lowwer, demand_zone_upper, demand_zone_lowwer, lst_idx_sell_secondry
		else:
			return signal, 0, 0, supply_zone_upper, supply_zone_lowwer, demand_zone_upper, demand_zone_lowwer, lst_idx_sell_secondry


	#ProfitFinder For Last Signal:

	def ProfitFinder(self, dataset_5M,dataset_1H, symbol, signal, index_candle, sigtype, pr_parameters, pr_config):
