...
_ADVANCED_COMMAND_FIELDS: Dict[CommandKind, set[str]] = {
    ...
    CommandKind.BACKTEST_REGIME_FILTERED_HYBRID: {
        ...
        "min_reward_risk",
        "buy_min_reward_risk",
        "sell_min_reward_risk",
        "max_sl_distance",
        "buy_max_sl_distance",
        "sell_max_sl_distance",
        "min_side_4h_room",
        "buy_min_side_4h_room",
        "sell_min_side_4h_room",
        "min_side_1d_room",
        "buy_min_side_1d_room",
        "sell_min_side_1d_room",
        "min_range_4h_width",
        "max_range_4h_width",
        "min_range_1d_width",
        "max_range_1d_width",
        "block_specialist_conflict",
        "max_booster_entropy",
        "min_booster_action_margin",
        "min_specialist_max_prob",
        "min_trades",
        "score_metric",
        "initial_capital",
        "units",
    ...
    CommandKind.AUDIT_PIVOT_PATTERN_IMAGE_TENSOR: {
        ...
        "full_scan",
        "max_scan_samples",
        "fail_on_reported_feature_nonfinite",
        "timeout_minutes",
    },
...
def descriptors(storage_root: "str | Path" = "datasets") -> List[CommandDescriptor]:
    ...
    items = [
        ...
        CommandDescriptor(
            kind=CommandKind.INSPECT_HYBRID_TELEMETRY_TENSOR,
            label="Inspect hybrid telemetry tensor",
            description=(
                "Create a visual HTML report for the Phase127 3D telemetry tensor: "
                "axis metadata, channel groups, targets and per-sample heatmaps."
            ),
            fields=[
                CommandField("symbol", "Symbol", "XAUUSD"),
                CommandField(
                ...
                CommandField("tensor_path", "Tensor path", ""),
                CommandField("flat_path", "Flat telemetry path", ""),
                CommandField("sample_indices", "Sample indices", "first,middle,last"),
                CommandField("time_buckets", "Time buckets", "50", kind="number"),
                CommandField("max_samples", "Max visual samples", "6", kind="number"),
                CommandField("timeout_minutes", "Give up after (minutes)", "60", kind="number"),
            ],
            slow=True,
            group="AI",
        ...
        CommandDescriptor(
            ...
            fields=[
                ...
                CommandField("stream_chunk_size", "Stream chunk size", "256", kind="number"),
        ...
        CommandDescriptor(
            kind=CommandKind.RUN_HYBRID_TENSOR_WALK_FORWARD_VALIDATION,
            label="Run tensor walk-forward validation",
            description=(
                "Train a fresh Phase129 tensor WaveNet/TCN on past months, select "
                "thresholds on past validation months, optionally choose NO_TRADE "
                "through Phase133C risk gates, and test the next unseen month."
            ),
            fields=[
                CommandField("symbol", "Symbol", "XAUUSD"),
                CommandField(
                ...
                CommandField(
                    "model_id", "Run/model id", "gold_hybrid_tensor_walkforward_wavenet_5m"
                ),
                CommandField(
                    "task",
                    "Task",
                ...
                CommandField("validation_months", "Validation months", "1", kind="number"),
                CommandField("purge_gap_bars", "Purge gap bars", "336", kind="number"),
                CommandField("decision_modes", "Decision modes", "score,both"),
                CommandField("meta_thresholds", "Meta threshold grid", "0.55,0.60,0.65,0.70"),
                CommandField("score_thresholds", "Score threshold grid", "0,0.05,0.10,0.20"),
                CommandField(
                    "default_meta_threshold", "Default meta threshold", "0.55", kind="number"
                ),
                CommandField(
                    "default_score_threshold", "Default score threshold", "0", kind="number"
                ),
                CommandField("min_trades", "Min validation trades", "10", kind="number"),
                CommandField(
                    "allow_no_trade",
                    "Allow validation NO TRADE",
                    "1",
                    kind="select",
                    options=("0", "1"),
                ),
                ...
                CommandField(
                    "min_validation_profit_factor", "Min validation PF", "1.10", kind="number"
                ),
                CommandField(
                    "max_validation_drawdown", "Max validation drawdown", "120", kind="number"
                ),
                CommandField("min_train_samples", "Min train samples", "200", kind="number"),
                CommandField(
                    "score_metric",
                    "Score metric",
                    "total_pnl",
                ...
                CommandField("max_train_samples", "Max train samples", "0", kind="number"),
                CommandField("batch_size", "Batch size", "64", kind="number"),
                CommandField("epochs", "Epochs per fold", "120", kind="number"),
                CommandField("learning_rate", "Learning rate", "0.001", kind="number"),
                CommandField("filters", "Conv filters", "48", kind="number"),
                CommandField("kernel_size", "Kernel size", "3", kind="number"),
                CommandField("n_layers", "Dilation layers", "5", kind="number"),
                ...
                CommandField("early_stopping_patience", "Early stop patience", "8", kind="number"),
                CommandField("verbose", "Training verbosity", "2", kind="number"),
                CommandField("initial_capital", "Initial capital ($)", "100", kind="number"),
                CommandField("units", "PnL units", "0.1", kind="number"),
                CommandField("timeout_minutes", "Give up after (minutes)", "480", kind="number"),
            ],
            slow=True,
            group="AI",
        ),
        CommandDescriptor(
            kind=CommandKind.ANALYZE_TENSOR_FAILURE_REGIMES,
            label="Analyze tensor failure regimes",
            description=(
                "Build the Phase136A diagnostic report for tensor/WaveNet failures: "
                "month, side, session, range, confidence, reward/risk and "
                "validation-to-test transfer tables."
            ),
            fields=[
                CommandField("symbol", "Symbol", "XAUUSD"),
                CommandField(
                ...
                CommandField(
                    "candidate_only", "Candidate rows only", "1", kind="select", options=("1", "0")
                ),
                CommandField("min_group_rows", "Min rows per regime", "20", kind="number"),
                CommandField("top_n", "Top/worst rows", "12", kind="number"),
                CommandField("quantile_bins", "Quantile bins", "5", kind="number"),
                CommandField("timeout_minutes", "Give up after (minutes)", "60", kind="number"),
            ],
            slow=True,
            group="AI",
        ...
        CommandDescriptor(
            kind=CommandKind.BACKTEST_REGIME_FILTERED_HYBRID,
            label="Backtest regime-filtered hybrid",
            description=(
                "Run Phase137A live-safe side-aware regime filters on hybrid candidates. "
                "BUY is not removed permanently: use allowed_sides and side-specific gates "
                "to test SELL-only, BUY-only, mixed, or strict-BUY hypotheses."
            ),
            fields=[
                CommandField("symbol", "Symbol", "XAUUSD"),
                CommandField(
                ...
                CommandField("start_month", "Start month", ""),
                CommandField("end_month", "End month", ""),
                CommandField("allowed_sides", "Allowed sides", "BUY,SELL"),
                CommandField("allowed_hours", "Allowed hours", ""),
                CommandField("buy_allowed_hours", "BUY allowed hours", ""),
                CommandField("sell_allowed_hours", "SELL allowed hours", ""),
                CommandField("min_candidate_confidence", "Min confidence", "0", kind="number"),
                CommandField("buy_min_confidence", "BUY min confidence", "-1", kind="number"),
                CommandField("sell_min_confidence", "SELL min confidence", "-1", kind="number"),
                CommandField("min_reward_risk", "Min reward/risk", "0", kind="number"),
                CommandField("buy_min_reward_risk", "BUY min reward/risk", "-1", kind="number"),
                CommandField("sell_min_reward_risk", "SELL min reward/risk", "-1", kind="number"),
                CommandField("max_sl_distance", "Max SL distance", "-1", kind="number"),
                CommandField("buy_max_sl_distance", "BUY max SL distance", "-1", kind="number"),
                CommandField("sell_max_sl_distance", "SELL max SL distance", "-1", kind="number"),
                CommandField("min_side_4h_room", "Min side 4H room", "0", kind="number"),
                CommandField("buy_min_side_4h_room", "BUY min side 4H room", "-1", kind="number"),
                CommandField("sell_min_side_4h_room", "SELL min side 4H room", "-1", kind="number"),
                CommandField("min_side_1d_room", "Min side 1D room", "0", kind="number"),
                CommandField("buy_min_side_1d_room", "BUY min side 1D room", "-1", kind="number"),
                CommandField("sell_min_side_1d_room", "SELL min side 1D room", "-1", kind="number"),
                CommandField("min_range_4h_width", "Min 4H width", "-1", kind="number"),
                CommandField("max_range_4h_width", "Max 4H width", "-1", kind="number"),
                CommandField("min_range_1d_width", "Min 1D width", "-1", kind="number"),
                CommandField("max_range_1d_width", "Max 1D width", "-1", kind="number"),
                CommandField(
                    "block_specialist_conflict",
                    "Block specialist conflict",
                    "0",
                    kind="select",
                    options=("0", "1"),
                ),
                CommandField("max_booster_entropy", "Max booster entropy", "-1", kind="number"),
                CommandField("min_booster_action_margin", "Min booster margin", "0", kind="number"),
                CommandField(
                    "min_specialist_max_prob", "Min specialist max prob", "0", kind="number"
                ),
                CommandField("min_trades", "Min trades", "10", kind="number"),
                CommandField(
                    "score_metric",
                ...
                CommandField("initial_capital", "Initial capital ($)", "100", kind="number"),
                CommandField("units", "PnL units", "0.1", kind="number"),
                CommandField("report_title", "Report title", "Regime-filtered hybrid replay"),
                CommandField("timeout_minutes", "Give up after (minutes)", "120", kind="number"),
            ],
            slow=True,
            group="AI",
        ...
        CommandDescriptor(
            kind=CommandKind.ANALYZE_TRADE_ANATOMY,
            label="Analyze trade anatomy",
            description=(
                "Build the Phase138A TP/SL and trade-anatomy diagnostic report: "
                "BUY/SELL damage, stop-loss concentration, TP/SL distances, "
                "reward/risk, hold-time and range context."
            ),
            fields=[
                CommandField("symbol", "Symbol", "XAUUSD"),
                CommandField(
                ...
                CommandField("end_month", "End month", ""),
                CommandField("allowed_sides", "Allowed sides", "BUY,SELL"),
                CommandField("min_group_rows", "Min rows per group", "20", kind="number"),
                CommandField("quantile_bins", "Quantile bins", "5", kind="number"),
                CommandField("top_n", "Top/worst rows", "15", kind="number"),
                CommandField("timeout_minutes", "Give up after (minutes)", "60", kind="number"),
            ],
            slow=True,
            group="AI",
        ...
        CommandDescriptor(
            kind=CommandKind.BACKTEST_BRACKET_RECALIBRATION,
            label="Backtest bracket recalibration",
            description=(
                "Run Phase139A TP/SL bracket recalibration replay on existing hybrid "
                "candidates. Tests TP caps, SL caps and reward/risk caps without "
            ...
            fields=[
                ...
                CommandField("end_month", "End month", ""),
                CommandField("allowed_sides", "Allowed sides", "BUY,SELL"),
                CommandField("max_tp_distances", "TP cap grid", "original,12,14,16,18"),
                CommandField("max_sl_distances", "SL cap grid", "original,20,23.56,30"),
                CommandField("max_reward_risks", "Max RR grid", "original,1.0,1.5,2.0"),
                CommandField("buy_max_tp_distance", "BUY max TP override", "-1", kind="number"),
                CommandField("buy_max_sl_distance", "BUY max SL override", "-1", kind="number"),
                CommandField("buy_max_reward_risk", "BUY max RR override", "-1", kind="number"),
                CommandField("sell_max_tp_distance", "SELL max TP override", "-1", kind="number"),
                CommandField("sell_max_sl_distance", "SELL max SL override", "-1", kind="number"),
                CommandField("sell_max_reward_risk", "SELL max RR override", "-1", kind="number"),
                CommandField("min_tp_distance", "Min TP distance", "0.1", kind="number"),
                CommandField("min_sl_distance", "Min SL distance", "0.1", kind="number"),
                CommandField("max_hold_bars", "Max hold bars", "48", kind="number"),
                CommandField(
                    "spread_mode", "Spread type", "pct", kind="select", options=("pct", "fixed")
                ),
                ...
                CommandField("initial_capital", "Initial capital ($)", "100", kind="number"),
                CommandField("units", "PnL units", "0.1", kind="number"),
                CommandField("report_title", "Report title", "TP/SL bracket recalibration replay"),
                CommandField("timeout_minutes", "Give up after (minutes)", "180", kind="number"),
            ],
            slow=True,
            group="AI",
        ...
        CommandDescriptor(
            kind=CommandKind.AUDIT_CANDIDATE_DIRECTION_ENTRY,
            label="Audit candidate direction/entry",
            description=(
                "Run Phase140A diagnostic replay on existing hybrid candidates: original "
                "vs BUY/SELL flipped side, entry delays 0/1/2/3, and side/month/session "
                "quality matrices. No training and no production approval."
            ),
            fields=[
                CommandField("symbol", "Symbol", "XAUUSD"),
                CommandField(
                ...
                CommandField("allowed_sides", "Allowed original sides", "BUY,SELL"),
                CommandField("entry_delays", "Entry delays", "0,1,2,3"),
                CommandField("side_modes", "Side modes", "original,flipped"),
                CommandField("execution_modes", "Execution modes", "independent,chronological"),
                CommandField("eval_frac", "Eval fraction", "1.00", kind="number"),
                CommandField("max_windows", "Max windows", "0", kind="number"),
                CommandField("start_month", "Start month", ""),
                CommandField("end_month", "End month", ""),
                ...
                CommandField("min_trades", "Min trades", "10", kind="number"),
                CommandField("min_group_trades", "Min group trades", "5", kind="number"),
                CommandField(
                    "score_metric",
                    "Score metric",
                    "total_pnl",
                ...
                CommandField("initial_capital", "Initial capital ($)", "100", kind="number"),
                CommandField("units", "PnL units", "0.1", kind="number"),
                CommandField("report_title", "Report title", "Candidate direction / entry audit"),
                CommandField("timeout_minutes", "Give up after (minutes)", "180", kind="number"),
            ],
            slow=True,
            group="AI",
        ...
        CommandDescriptor(
            kind=CommandKind.TRAIN_PIVOT_PATTERN_RECOGNITION,
            label="Train pivot pattern recognition",
            description=(
                "Run Phase141A top/bottom reversal-zone pattern recognition: build causal "
                "5M/4H/1D features, label pivot zones, select useful features, train the "
                "model, and run walk-forward backtest. Research only."
            ),
            fields=[
                CommandField("symbol", "Canonical symbol", "XAUUSD"),
                CommandField(
                    "dataset",
                    "Dataset",
                    "5M" if "5M" in datasets else (datasets[0] if datasets else "5M"),
                ...
                CommandField(
                    ...
                    "storage",
                    kind="select",
                    options=("storage", "yahoo", "files"),
                ),
                CommandField("yahoo_symbol", "Yahoo gold symbol", "GC=F"),
                CommandField("daily_range", "Yahoo 1D range", "5y"),
                CommandField("hourly_range", "Yahoo 1H range", "730d"),
                CommandField("five_range", "Yahoo 5M range", "60d"),
                CommandField("five_timeframe", "Storage 5M timeframe", "5M"),
                CommandField("hourly_timeframe", "Storage 1H timeframe", "1H"),
                CommandField("h4_timeframe", "Storage 4H timeframe", "4H"),
                CommandField("daily_timeframe", "Storage 1D timeframe", "1D"),
                CommandField("daily_path", "1D file path", ""),
                CommandField("hourly_path", "1H file path", ""),
                CommandField("h4_path", "4H file path", ""),
                CommandField("five_path", "5M file path", ""),
                CommandField("model_id", "Model id", "gold_pivot_pattern_recognition_5m"),
                CommandField(
                    "model_kind",
                    "Model kind",
                    "auto",
                    kind="select",
                    options=("auto", "sklearn_hgb", "centroid"),
                ),
                CommandField("lookahead_bars", "Pivot lookahead bars", "48", kind="number"),
                CommandField("pivot_move_atr", "Pivot move ATR", "0.75", kind="number"),
                CommandField("pivot_zone_atr", "Pivot zone ATR", "0.35", kind="number"),
                CommandField("recent_window_bars", "Recent zone window", "48", kind="number"),
                CommandField("min_direction_edge", "Min direction edge", "1.10", kind="number"),
                CommandField("max_features", "Max selected features", "38", kind="number"),
                CommandField("train_days_min", "Min train days", "20", kind="number"),
                CommandField("validation_days", "Validation days", "5", kind="number"),
                CommandField("test_days", "Test days", "5", kind="number"),
                CommandField("purge_hours", "Purge hours", "4", kind="number"),
                CommandField("buy_thresholds", "BUY threshold grid", "0.45,0.55,0.65"),
                CommandField("sell_thresholds", "SELL threshold grid", "0.45,0.55,0.65"),
                CommandField("margins", "Margin grid", "0,0.05"),
                CommandField("tp_multipliers", "TP ATR multipliers", "0.75,1.0,1.25"),
                CommandField("sl_multipliers", "SL ATR multipliers", "0.50,0.75"),
                CommandField("hold_bars", "Max hold bars grid", "24,48"),
                CommandField("min_validation_trades", "Min validation trades", "5", kind="number"),
                CommandField(
                    "score_metric",
                    "Validation score metric",
                    "net_profit",
                    kind="select",
                    options=("net_profit", "profit_factor", "final_balance", "drawdown_adjusted"),
                ),
                CommandField(
                    "spread_mode", "Spread type", "pct", kind="select", options=("pct", "fixed")
                ),
                ...
                CommandField(
                    "save_model", "Save model artifact", "1", kind="select", options=("1", "0")
                ),
                CommandField(
                    "save_data", "Save source data snapshot", "1", kind="select", options=("1", "0")
                ),
                CommandField(
                    "report_title", "Report title", "Pivot pattern recognition walk-forward"
                ),
                CommandField("timeout_minutes", "Give up after (minutes)", "240", kind="number"),
            ],
            slow=True,
        ...
        CommandDescriptor(
            kind=CommandKind.BUILD_PIVOT_PATTERN_TENSOR,
            label="Build pivot pattern tensor",
            description=(
                "Build Phase142A 3D pivot/top-bottom tensor from project storage. "
                "Stored X shape is [samples, 288, channels]; Keras batches are "
                "[batch, 288, channels]."
            ),
            fields=[
                CommandField("symbol", "Symbol", "XAUUSD"),
                ...
                CommandField("recent_window_bars", "Recent zone window", "48", kind="number"),
                CommandField("min_direction_edge", "Min direction edge", "1.10", kind="number"),
                CommandField("tensor_window", "Tensor window", "288", kind="number"),
                CommandField("sample_stride", "Sample stride", "1", kind="number"),
                CommandField("max_samples", "Max samples", "0", kind="number"),
                CommandField(
                    "dtype",
                ...
                CommandField("max_tensor_mb", "Max tensor MB", "8192", kind="number"),
                CommandField("output_name", "Output name", "pivot_pattern_tensor_latest"),
                CommandField("timeout_minutes", "Give up after (minutes)", "180", kind="number"),
            ],
            slow=True,
            group="AI",
        ...
        CommandDescriptor(
            kind=CommandKind.TRAIN_PIVOT_PATTERN_WAVENET,
            label="Train pivot WaveNet pattern model",
            description=(
                "Train Phase142A TensorFlow/Keras WaveNet/TCN on pivot tensor. "
                "Input batches are [batch, 288, channels]."
            ),
            fields=[
                CommandField("symbol", "Symbol", "XAUUSD"),
                CommandField(
                ...
                CommandField("tensor_path", "Pivot tensor path", ""),
                CommandField("model_id", "Model id", "gold_pivot_pattern_wavenet_5m"),
                CommandField("train_frac", "Train fraction", "0.70", kind="number"),
                CommandField("val_frac", "Validation fraction", "0.15", kind="number"),
                ...
                CommandField("stream_chunk_size", "Stream chunk size", "256", kind="number"),
                CommandField("batch_size", "Batch size", "64", kind="number"),
                CommandField("epochs", "Epochs", "120", kind="number"),
                CommandField("learning_rate", "Learning rate", "0.001", kind="number"),
                CommandField("filters", "Conv filters", "48", kind="number"),
                CommandField("kernel_size", "Kernel size", "3", kind="number"),
                CommandField("n_layers", "Dilation layers", "6", kind="number"),
                CommandField("n_blocks", "Residual blocks", "2", kind="number"),
                CommandField("dense_units", "Dense units", "96", kind="number"),
                CommandField("dropout", "Dropout", "0.20", kind="number"),
                CommandField("action_loss_weight", "Action loss weight", "1.0", kind="number"),
                CommandField("top_loss_weight", "Top loss weight", "0.5", kind="number"),
                CommandField("bottom_loss_weight", "Bottom loss weight", "0.5", kind="number"),
                CommandField("r_loss_weight", "R loss weight", "0.5", kind="number"),
                CommandField(
                    "class_weight", "Class weights", "auto", kind="select", options=("auto", "off")
                ),
                CommandField("buy_threshold", "BUY threshold", "0.55", kind="number"),
                CommandField("sell_threshold", "SELL threshold", "0.55", kind="number"),
                CommandField("min_margin", "Min margin", "0.05", kind="number"),
                CommandField("min_buy_r", "Min BUY R", "0", kind="number"),
                CommandField("min_sell_r", "Min SELL R", "0", kind="number"),
                CommandField(
                    "monitor_metric",
                    "Monitor metric",
                    "auto",
                ...
                CommandField("early_stopping_patience", "Early stop patience", "12", kind="number"),
                CommandField(
                    "checkpoint_each_epoch",
                ...
                CommandField("batch_log_every", "Print/log every N batches", "10", kind="number"),
                CommandField("batch_log_file", "Batch log file", ""),
                CommandField("save_model", "Save model", "1", kind="select", options=("1", "0")),
                CommandField("report_title", "Report title", "Pivot WaveNet training"),
                CommandField("timeout_minutes", "Give up after (minutes)", "480", kind="number"),
            ],
            slow=True,
            group="AI",
        ...
        CommandDescriptor(
            kind=CommandKind.BACKTEST_PIVOT_PATTERN_WAVENET,
            label="Backtest pivot WaveNet pattern model",
            description=(
                "Replay a trained Phase142A Keras WaveNet on the pivot tensor "
                "with TP/SL and $100 balance simulation."
            ),
            fields=[
                CommandField("symbol", "Symbol", "XAUUSD"),
                CommandField(
                ...
                CommandField("tensor_path", "Pivot tensor path", ""),
                CommandField("flat_path", "Pivot flat path", ""),
                CommandField("model_id", "Model id", "gold_pivot_pattern_wavenet_5m"),
                CommandField("model_version", "Model version", "0", kind="number"),
                CommandField("model_path", "Model path override", ""),
                CommandField("record_path", "Record path override", ""),
                CommandField("eval_frac", "Eval fraction", "1.0", kind="number"),
                CommandField("max_windows", "Max windows", "0", kind="number"),
                CommandField("buy_threshold", "BUY threshold", "-1", kind="number"),
                CommandField("sell_threshold", "SELL threshold", "-1", kind="number"),
                CommandField("min_margin", "Min margin", "-1", kind="number"),
                CommandField("top_threshold", "Top threshold", "0.50", kind="number"),
                CommandField("bottom_threshold", "Bottom threshold", "0.50", kind="number"),
                ...
                CommandField("initial_capital", "Initial capital ($)", "100", kind="number"),
                CommandField("risk_per_trade", "Risk per trade", "0.01", kind="number"),
                CommandField("report_title", "Report title", "Pivot WaveNet backtest"),
                CommandField("timeout_minutes", "Give up after (minutes)", "180", kind="number"),
            ],
            slow=True,
            group="AI",
        ...
        CommandDescriptor(
            kind=CommandKind.RUN_PIVOT_PATTERN_WAVENET_WALK_FORWARD,
            label="Run pivot WaveNet walk-forward",
            description=(
                "Train a fresh Phase142A Keras WaveNet per fold, select thresholds "
                "on validation, and test the next unseen window."
            ),
            fields=[
                CommandField("symbol", "Symbol", "XAUUSD"),
                CommandField(
                ...
                CommandField("tensor_path", "Pivot tensor path", ""),
                CommandField("flat_path", "Pivot flat path", ""),
                CommandField("model_id", "Model id", "gold_pivot_pattern_wavenet_wf_5m"),
                CommandField("train_days_min", "Min train days", "120", kind="number"),
                CommandField("validation_days", "Validation days", "20", kind="number"),
                CommandField("test_days", "Test days", "20", kind="number"),
                CommandField("purge_hours", "Purge hours", "4", kind="number"),
                CommandField("max_folds", "Max folds", "0", kind="number"),
                CommandField("max_train_samples", "Max train samples", "0", kind="number"),
                CommandField("batch_size", "Batch size", "64", kind="number"),
                CommandField("epochs", "Epochs/fold", "80", kind="number"),
                CommandField("learning_rate", "Learning rate", "0.001", kind="number"),
                CommandField("filters", "Conv filters", "48", kind="number"),
                CommandField("kernel_size", "Kernel size", "3", kind="number"),
                CommandField("n_layers", "Dilation layers", "6", kind="number"),
                ...
                CommandField("early_stopping_patience", "Early stop patience", "10", kind="number"),
                CommandField("buy_thresholds", "BUY threshold grid", "0.45,0.55,0.65"),
                CommandField("sell_thresholds", "SELL threshold grid", "0.45,0.55,0.65"),
                CommandField("margins", "Margin grid", "0,0.05"),
                CommandField("top_thresholds", "Top threshold grid", "0.45,0.55,0.65"),
                CommandField("bottom_thresholds", "Bottom threshold grid", "0.45,0.55,0.65"),
                CommandField("min_r_values", "Min R grid", "-0.25,0,0.25"),
                CommandField("tp_multipliers", "TP ATR multipliers", "0.75,1.0,1.25"),
                CommandField("sl_multipliers", "SL ATR multipliers", "0.50,0.75"),
                CommandField("hold_bars", "Max hold bars grid", "24,48"),
                CommandField("min_validation_trades", "Min validation trades", "10", kind="number"),
                CommandField(
                    "score_metric",
                    "Validation score metric",
                    "drawdown_adjusted",
                ...
                CommandField("risk_per_trade", "Risk per trade", "0.01", kind="number"),
                CommandField(
                    "report_title", "Report title", "Pivot WaveNet walk-forward validation"
                ),
        ...
        CommandDescriptor(
            kind=CommandKind.BUILD_PIVOT_PATTERN_IMAGE_TENSOR,
            label="Build pivot image tensor",
            description=(
                "Build Phase143A 4D image/volume tensor for Conv2D/Conv3D: "
                "[samples, window, 5M features, 4H/1D features]."
            ),
            fields=[
                CommandField("symbol", "Symbol", "XAUUSD"),
                CommandField(
                ...
                CommandField("recent_window_bars", "Recent zone window", "48", kind="number"),
                CommandField("min_direction_edge", "Min direction edge", "1.10", kind="number"),
                CommandField("window_size", "Window size", "100", kind="number"),
                CommandField("sample_stride", "Sample stride", "1", kind="number"),
                CommandField("max_samples", "Max samples", "0", kind="number"),
                CommandField("max_5m_features", "Max 5M features", "24", kind="number"),
                CommandField("max_htf_features", "Max 4H/1D features", "16", kind="number"),
                CommandField(
                    "dtype",
                    "Tensor dtype",
                    "float16",
                ...
                CommandField("chunk_size", "Build chunk size", "256", kind="number"),
                CommandField("max_tensor_mb", "Max tensor MB", "8192", kind="number"),
                CommandField(
                    "axis_normalization",
                    "Axis normalization",
                    "robust",
                    kind="select",
                    options=("robust", "standard", "none"),
                ),
                CommandField("feature_clip", "Feature clip", "8", kind="number"),
                CommandField("interaction_clip", "Interaction clip", "32", kind="number"),
                CommandField("output_name", "Output name", "pivot_pattern_image_tensor_latest"),
                CommandField("timeout_minutes", "Give up after (minutes)", "240", kind="number"),
            ],
            slow=True,
            group="AI",
        ...
        CommandDescriptor(
            kind=CommandKind.AUDIT_PIVOT_PATTERN_IMAGE_TENSOR,
            label="Audit pivot image tensor health",
            description=(
                "Validate Phase143A 4D pivot image tensor before training: shape, "
                "metadata alignment, targets, finite scan, clipping bounds, "
                "and builder diagnostics."
            ),
            fields=[
                CommandField("symbol", "Symbol", "XAUUSD"),
                CommandField(
                ...
                CommandField("tensor_path", "Image tensor path", ""),
                CommandField("meta_path", "Image tensor meta path", ""),
                CommandField("flat_path", "Image tensor flat path", ""),
                CommandField(
                    "builder_report",
                    "Builder report JSON",
                    "run_logs/pivot_pattern_image_tensor/latest.json",
                ),
                CommandField("chunk_size", "Scan chunk size", "256", kind="number"),
                CommandField(
                    "full_scan", "Full tensor scan", "1", kind="select", options=("1", "0")
                ),
                CommandField("max_scan_samples", "Max scan samples", "0", kind="number"),
                CommandField(
                    "fail_on_reported_feature_nonfinite",
                    "Fail on raw feature nonfinite",
                    "0",
                    kind="select",
                    options=("0", "1"),
                ),
        ...
        CommandDescriptor(
            kind=CommandKind.TRAIN_PIVOT_PATTERN_IMAGE_CNN,
            label="Train pivot image CNN",
            description=(
                "Train Phase143A Conv2D or Conv3D model on the 4D pivot image tensor. "
                "Conv2D batch shape is [batch, window, 5M features, 4H/1D features]."
            ),
            fields=[
                CommandField("symbol", "Symbol", "XAUUSD"),
                CommandField(
                ...
                CommandField("tensor_path", "Image tensor path", ""),
                CommandField("meta_path", "Image tensor meta path", ""),
                CommandField("model_id", "Model id", "gold_pivot_pattern_image_cnn_5m"),
                CommandField(
                    "model_kind",
                    "Model kind",
                    "conv2d",
                    kind="select",
                    options=("conv2d", "conv3d"),
                ),
                CommandField("train_frac", "Train fraction", "0.70", kind="number"),
                CommandField("val_frac", "Validation fraction", "0.15", kind="number"),
                CommandField("purge_gap", "Purge gap samples", "336", kind="number"),
                ...
                CommandField("epochs", "Epochs", "120", kind="number"),
                CommandField("learning_rate", "Learning rate", "0.001", kind="number"),
                CommandField("filters", "Conv filters", "32", kind="number"),
                CommandField("kernel_size", "Kernel size", "3", kind="number"),
                CommandField("dense_units", "Dense units", "96", kind="number"),
                CommandField("dropout", "Dropout", "0.20", kind="number"),
                CommandField("action_loss_weight", "Action loss weight", "1.0", kind="number"),
                ...
                CommandField("batch_log_file", "Batch log file", ""),
                CommandField("save_model", "Save model", "1", kind="select", options=("1", "0")),
                CommandField("report_title", "Report title", "Pivot image CNN training"),
                CommandField("timeout_minutes", "Give up after (minutes)", "480", kind="number"),
            ],
            slow=True,
            group="AI",
        ...
        CommandDescriptor(
            kind=CommandKind.TRAIN_PIVOT_PATTERN_IMAGE_WAVENET,
            label="Train advanced pivot image WaveNet",
            description=(
                "Train Phase144A advanced 4D image WaveNet with separate 5M/HTF "
                "branches, multi-scale kernels, gated tanh dilated residual blocks, "
                "SE attention, and temporal self-attention."
            ),
            fields=[
                CommandField("symbol", "Symbol", "XAUUSD"),
                CommandField(
                ...
                CommandField("tensor_path", "Image tensor path", ""),
                CommandField("meta_path", "Image tensor meta path", ""),
                CommandField("model_id", "Model id", "gold_pivot_pattern_image_wavenet_5m"),
                CommandField("train_frac", "Train fraction", "0.70", kind="number"),
                CommandField("val_frac", "Validation fraction", "0.15", kind="number"),
                CommandField("purge_gap", "Purge gap samples", "336", kind="number"),
                ...
                CommandField("spatial_filters", "Spatial filters", "32", kind="number"),
                CommandField("branch_filters", "5M/HTF branch filters", "32", kind="number"),
                CommandField("temporal_filters", "Temporal filters", "64", kind="number"),
                CommandField("spatial_kernels", "Spatial kernels", "1,3,5"),
                CommandField("temporal_kernels", "Temporal kernels", "3,5,9"),
                CommandField("dilations", "WaveNet dilations", "1,2,4,8,16,32"),
                CommandField("residual_blocks", "Residual blocks", "2", kind="number"),
                CommandField("attention_heads", "Attention heads", "4", kind="number"),
                CommandField("attention_key_dim", "Attention key dim", "16", kind="number"),
                CommandField("se_ratio", "SE ratio", "8", kind="number"),
                CommandField("dense_units", "Dense units", "128", kind="number"),
                CommandField("dropout", "Dropout", "0.25", kind="number"),
                CommandField(
                    "activation",
                    "Activation",
                    "tanh",
                    kind="select",
                    options=("tanh", "swish", "gelu"),
                ),
                CommandField("action_loss_weight", "Action loss weight", "1.0", kind="number"),
                CommandField("top_loss_weight", "Top loss weight", "0.5", kind="number"),
                CommandField("bottom_loss_weight", "Bottom loss weight", "0.5", kind="number"),
                ...
                CommandField("save_model", "Save model", "1", kind="select", options=("1", "0")),
                CommandField(
                    "report_title", "Report title", "Advanced Pivot Image WaveNet training"
                ),
                CommandField("timeout_minutes", "Give up after (minutes)", "720", kind="number"),
            ],
            slow=True,
        ...
        CommandDescriptor(
            kind=CommandKind.BUILD_PIVOT_PATTERN_SEQUENCE_TENSOR,
            label="Build pivot sequence tensor",
            description=(
                "Build Phase145A Option A tensor: [samples, window, features]. "
                "Concatenates 5M, closed 4H/1D and source 5M feature columns."
            ),
            fields=[
                CommandField("symbol", "Symbol", "XAUUSD"),
                CommandField(
                ...
                CommandField(
                    "source_5m_feature_path",
                    "Optional source 5M feature parquet/csv",
                    "",
                ),
                CommandField("max_features", "Max features", "0", kind="number"),
                CommandField(
                    "dtype",
                    "Tensor dtype",
                    "float16",
                ...
                CommandField("feature_clip", "Feature clip", "8", kind="number"),
                CommandField("chunk_size", "Build chunk size", "512", kind="number"),
                CommandField("max_tensor_mb", "Max tensor MB", "8192", kind="number"),
                CommandField("output_name", "Output name", "pivot_pattern_sequence_tensor_latest"),
                CommandField("timeout_minutes", "Give up after (minutes)", "180", kind="number"),
            ],
            slow=True,
            group="AI",
        ...
        CommandDescriptor(
            kind=CommandKind.AUDIT_PIVOT_PATTERN_SEQUENCE_TENSOR,
            label="Audit pivot sequence tensor health",
            description=(
                "Audit Phase145A Option A [samples, window, features] tensor health, "
                "including metadata alignment, feature/source counts, finite scan, and clip bounds."
            ),
            fields=[
                CommandField("symbol", "Symbol", "XAUUSD"),
                CommandField(
                ...
                CommandField(
                    "builder_report",
                    "Builder report JSON",
                    "run_logs\\pivot_pattern_sequence_tensor\\latest.json",
                ),
                CommandField("chunk_size", "Scan chunk size", "512", kind="number"),
                CommandField("full_scan", "Full tensor scan", "1", kind="select", options=("1", "0")),
                CommandField("max_scan_samples", "Max scan samples", "0", kind="number"),
                CommandField(
                    "fail_on_reported_feature_nonfinite",
        ...
        CommandDescriptor(
            kind=CommandKind.TRAIN_PIVOT_PATTERN_SEQUENCE_WAVENET,
            label="Train pivot sequence WaveNet",
            description=(
                "Train Phase145A advanced Conv1D/WaveNet on [batch, window, features] "
                "with concatenated 5M + 4H/1D features."
            ),
            fields=[
                CommandField("symbol", "Symbol", "XAUUSD"),
                CommandField(
                ...
                CommandField("stream_chunk_size", "Stream chunk size", "512", kind="number"),
                CommandField("batch_size", "Batch size", "16", kind="number"),
                CommandField("epochs", "Epochs", "80", kind="number"),
                CommandField("learning_rate", "Learning rate", "0.0005", kind="number"),
                CommandField("random_seed", "Random seed", "20260919", kind="number"),
                CommandField("branch_filters", "Branch filters", "48", kind="number"),
                CommandField("temporal_filters", "Temporal filters", "96", kind="number"),
                CommandField("temporal_kernels", "Temporal kernels", "3,5,9"),
                CommandField("dilations", "WaveNet dilations", "1,2,4,8,16,32"),
                CommandField("residual_blocks", "Residual blocks", "2", kind="number"),
                CommandField("attention_heads", "Attention heads", "4", kind="number"),
                ...
                CommandField("save_model", "Save model", "1", kind="select", options=("1", "0")),
                CommandField(
                    "report_title", "Report title", "Advanced Pivot Sequence WaveNet training"
                ),
                CommandField("timeout_minutes", "Give up after (minutes)", "720", kind="number"),
            ],
            slow=True,
        ...
        CommandDescriptor(
            ...
            fields=[
                ...
                CommandField(
                    ...
                    "test",
                    kind="select",
                    options=("train", "validation", "test", "all", "tail"),
                ),
        ...
        CommandDescriptor(
            kind=CommandKind.TRAIN_PIVOT_PATTERN_SEQUENCE_WAVENET_OPTION_B,
            label="Train pivot sequence WaveNet Option B",
            description=(
                "Train Phase147A Option B grouped-input sequence WaveNet with separate "
                "generated 5M/session, source 5M telemetry, and HTF context inputs."
            ),
            fields=[
                CommandField("symbol", "Symbol", "XAUUSD"),
                CommandField(
                ...
                CommandField("random_seed", "Random seed", "20260919", kind="number"),
                CommandField("branch_filters", "Branch filters", "48", kind="number"),
                CommandField("branch_target_features", "Expanded features per branch", "0", kind="number"),
                CommandField(
                    "feature_augmentation_mode",
                ...
                CommandField("feature_augmentation_clip", "Augmentation clip", "8", kind="number"),
                                CommandField("zero_feature_names", "Zero feature names", ""),
                CommandField("zero_feature_file", "Zero feature file", ""),
                CommandField("zero_feature_groups", "Zero feature groups", ""),
CommandField("temporal_filters", "Temporal filters", "96", kind="number"),
                CommandField("temporal_kernels", "Temporal kernels", "3,5,9"),
                CommandField("dilations", "WaveNet dilations", "1,2,4,8,16,32"),
                CommandField("residual_blocks", "Residual blocks", "2", kind="number"),
                ...
                CommandField("batch_log_file", "Batch log file", ""),
                CommandField("save_model", "Save model", "1", kind="select", options=("1", "0")),
                CommandField("report_title", "Report title", "Option B Pivot Sequence WaveNet training"),
                CommandField("timeout_minutes", "Give up after (minutes)", "720", kind="number"),
            ],
            slow=True,
            group="AI",
        ...
        CommandDescriptor(
            ...
            description=(
                "Run Step 4 range-aware prediction/backtest archive: model predictions, "
                "range-derived TP/SL, trades, monthly/side summaries, and full CSV/parquet outputs."
            ),
            fields=[
                CommandField("symbol", "Symbol", "XAUUSD"),
                CommandField(
                ...
                CommandField("min_sell_r", "Min SELL R", "-999", kind="number"),
                CommandField(
                    "range_source", "Range source", "auto", kind="select", options=("auto", "flat", "models")
                ),
                CommandField(
                    "range_bracket_mode",
                    "Range bracket mode",
                    "range_capped",
                    kind="select",
                    options=("range", "range_capped", "atr"),
                ),
                CommandField(
                    "range_fallback", "Range fallback", "skip", kind="select", options=("skip", "atr")
                ),
                CommandField("range_1d_model_id", "1D range model id", "gold_range_1d"),
                CommandField("range_4h_model_id", "4H range model id", "gold_range_4h"),
                CommandField("range_1d_version", "1D range version", "0", kind="number"),
                CommandField("range_4h_version", "4H range version", "0", kind="number"),
                CommandField("use_1d_tp_cap", "Use 1D TP cap", "1", kind="select", options=("1", "0")),
                CommandField("min_range_4h_room", "Min 4H range room", "0", kind="number"),
                CommandField("min_range_1d_room", "Min 1D range room", "0", kind="number"),
                CommandField("min_tp_distance", "Min TP distance", "0.5", kind="number"),
                CommandField("min_sl_distance", "Min SL distance", "0.5", kind="number"),
                CommandField("max_tp_atr", "Max TP ATR cap", "2.0", kind="number"),
                CommandField("max_sl_atr", "Max SL ATR cap", "1.25", kind="number"),
                CommandField("atr_tp_multiplier", "ATR fallback TP mult", "0.75", kind="number"),
                CommandField("atr_sl_multiplier", "ATR fallback SL mult", "0.75", kind="number"),
                CommandField("hold_bars", "Max hold bars", "48", kind="number"),
                CommandField(
                    "spread_mode", "Spread mode", "pct", kind="select", options=("pct", "fixed")
                ),
                ...
                CommandField("initial_capital", "Initial capital", "100", kind="number"),
                CommandField("risk_per_trade", "Risk per trade", "0.01", kind="number"),
                CommandField("report_title", "Report title", "Phase148A range-aware sequence WaveNet archive"),
                CommandField("timeout_minutes", "Give up after (minutes)", "360", kind="number"),
            ],
            slow=True,
            group="AI",
        ...
        CommandDescriptor(
            kind=CommandKind.AUDIT_PIVOT_SEQUENCE_RANGE_BRACKET_GEOMETRY,
            label="Audit sequence range bracket geometry",
            description=(
                "Replay Phase148 prediction archives under alternative TP/SL geometry policies "
                "without retraining or changing model predictions."
            ),
            fields=[
                CommandField(
                    "predictions_path",
                    "Predictions archive path",
                    "run_logs/pivot_pattern_sequence_wavenet_range_archive_validation/latest_predictions.parquet",
                ),
                CommandField(
                    "flat_path",
                    "Sequence flat parquet path",
                    "datasets/processed/XAUUSD/5M/pivot_pattern_sequence_flat_latest.parquet",
                ),
                CommandField("split_name", "Split label", "validation"),
                CommandField("allowed_sides", "Allowed sides", "BUY,SELL"),
                CommandField("range_bracket_modes", "Bracket modes", "atr,range_capped"),
                CommandField("range_fallbacks", "Range fallbacks", "skip"),
                CommandField("use_1d_tp_caps", "Use 1D TP cap grid", "1,0"),
                CommandField("min_tp_distances", "Min TP distances", "0.5,2,5"),
                CommandField("min_sl_distances", "Min SL distances", "0.5,2,5,10"),
                CommandField("max_tp_atrs", "Max TP ATR caps", "2.0,0"),
                CommandField("max_sl_atrs", "Max SL ATR caps", "1.25,2.0"),
                CommandField("min_range_4h_rooms", "Min 4H rooms", "0"),
                CommandField("min_range_1d_rooms", "Min 1D rooms", "0"),
                CommandField("same_bar_policies", "Same-bar policies", "stop_first"),
                CommandField("atr_tp_multiplier", "ATR TP multiplier", "0.75", kind="number"),
                CommandField("atr_sl_multiplier", "ATR SL multiplier", "0.75", kind="number"),
                CommandField("hold_bars", "Max hold bars", "48", kind="number"),
                CommandField(
                    "spread_mode", "Spread mode", "pct", kind="select", options=("pct", "fixed")
                ),
                ...
                CommandField("initial_capital", "Initial capital", "100", kind="number"),
                CommandField("risk_per_trade", "Risk per trade", "0.01", kind="number"),
                CommandField("min_trades", "Minimum trades", "30", kind="number"),
                CommandField(
                    "score_metric",
                    "Score metric",
                    "drawdown_adjusted",
                    kind="select",
                    options=("drawdown_adjusted", "total_pnl", "profit_factor"),
                ),
                CommandField("pass_min_profit_factor", "Pass min PF", "1.05", kind="number"),
                CommandField("pass_min_final_balance", "Pass min final balance", "100", kind="number"),
                CommandField("pass_max_drawdown", "Pass max drawdown", "25", kind="number"),
                CommandField("output_dir", "Output dir", "run_logs/pivot_sequence_range_bracket_geometry"),
                CommandField("report_title", "Report title", "Phase150A range bracket geometry audit"),
                CommandField("timeout_minutes", "Give up after (minutes)", "240", kind="number"),
            ],
            slow=True,
            group="AI",
        ...
        CommandDescriptor(
            kind=CommandKind.RUN_PIVOT_SEQUENCE_OPTION_B_SEED_TRANSFER,
            label="Run Option B seed transfer validation",
            description=(
                "Train multiple seeded raw Option B models, evaluate validation/test with fixed "
                "ATR replay, and select only by validation."
            ),
            fields=[
                CommandField("symbol", "Symbol", "XAUUSD"),
                CommandField(
                ...
                CommandField("meta_path", "Sequence tensor meta path", ""),
                CommandField("flat_path", "Sequence flat parquet path", ""),
                CommandField("model_id_prefix", "Model id prefix", "gold_pivot_pattern_sequence_wavenet_option_b_seed_audit_5m"),
                CommandField("seeds", "Seeds", "20260919,20260920,20260921"),
                CommandField("max_samples", "Max samples", "24000", kind="number"),
                CommandField("train_frac", "Train fraction", "0.70", kind="number"),
                CommandField("val_frac", "Validation fraction", "0.15", kind="number"),
                CommandField("purge_gap", "Purge gap samples", "336", kind="number"),
                ...
                CommandField("early_stopping_patience", "Early stop patience", "8", kind="number"),
                CommandField("learning_rate", "Learning rate", "0.0005", kind="number"),
                CommandField("batch_size", "Train batch size", "16", kind="number"),
                CommandField("branch_filters", "Branch filters", "48", kind="number"),
                CommandField("temporal_filters", "Temporal filters", "96", kind="number"),
                CommandField("buy_threshold", "BUY threshold", "0.34", kind="number"),
                CommandField("sell_threshold", "SELL threshold", "0.34", kind="number"),
                CommandField("min_margin", "Min margin", "0", kind="number"),
                CommandField("top_threshold", "Top threshold", "0", kind="number"),
                CommandField("bottom_threshold", "Bottom threshold", "0", kind="number"),
                ...
                CommandField("hold_bars", "Max hold bars", "48", kind="number"),
                CommandField("selection_metric", "Selection metric", "drawdown_adjusted", kind="select", options=("drawdown_adjusted", "validation_pnl", "validation_pf")),
                CommandField("validation_min_trades", "Validation min trades", "30", kind="number"),
                CommandField("validation_min_profit_factor", "Validation min PF", "1.05", kind="number"),
                CommandField("validation_min_final_balance", "Validation min final", "100", kind="number"),
                CommandField("test_min_profit_factor", "Test min PF", "1.05", kind="number"),
                CommandField("test_min_final_balance", "Test min final", "100", kind="number"),
                CommandField("output_dir", "Output dir", "run_logs/pivot_sequence_option_b_seed_transfer"),
                CommandField("skip_existing_models", "Skip existing models", "1", kind="select", options=("1", "0")),
        ...
        CommandDescriptor(
            kind=CommandKind.AUDIT_PIVOT_LABEL_TARGET_STABILITY,
            label="Audit pivot label/target stability",
            description=(
                "Audit pivot target label drift across train/validation/test and run "
                "label-parameter sensitivity diagnostics before further training."
            ),
            fields=[
                CommandField("symbol", "Symbol", "XAUUSD"),
                CommandField(
                ...
                CommandField(
                    "flat_path",
                    "Pivot sequence flat path",
                    "datasets/processed/XAUUSD/5M/pivot_pattern_sequence_flat_latest.parquet",
                ),
                CommandField("meta_path", "Pivot sequence meta path", ""),
                CommandField("max_samples", "Max sampled windows", "24000", kind="number"),
                CommandField("train_frac", "Train fraction", "0.70", kind="number"),
                CommandField("val_frac", "Validation fraction", "0.15", kind="number"),
                CommandField("purge_gap", "Purge gap samples", "336", kind="number"),
                CommandField("window_size", "Window size", "100", kind="number"),
                CommandField("lookahead_bars", "Current lookahead bars", "48", kind="number"),
                CommandField("pivot_move_atr", "Current pivot move ATR", "0.75", kind="number"),
                CommandField("pivot_zone_atr", "Current pivot zone ATR", "0.35", kind="number"),
                CommandField("recent_window_bars", "Recent window bars", "48", kind="number"),
                CommandField("min_direction_edge", "Current min direction edge", "1.10", kind="number"),
                CommandField("sensitivity_lookahead_bars", "Sensitivity lookaheads", "24,48,72"),
                CommandField("sensitivity_pivot_move_atrs", "Sensitivity move ATRs", "0.5,0.75,1.0"),
                CommandField("sensitivity_pivot_zone_atrs", "Sensitivity zone ATRs", "0.25,0.35,0.5"),
                CommandField("sensitivity_min_direction_edges", "Sensitivity direction edges", "1.0,1.1,1.25"),
                CommandField("max_sensitivity_combinations", "Max sensitivity combos", "40", kind="number"),
                CommandField("psi_warning_threshold", "PSI warning threshold", "0.20", kind="number"),
                CommandField("output_dir", "Output dir", "run_logs/pivot_label_target_stability"),
                CommandField("report_title", "Report title", "Phase152A pivot label target stability audit"),
                CommandField("timeout_minutes", "Give up after (minutes)", "240", kind="number"),
            ],
            slow=True,
            group="AI",
        ...
        CommandDescriptor(
            kind=CommandKind.RUN_PIVOT_TARGET_REDESIGN_CANDIDATE_AUDIT,
            label="Build/audit pivot target redesign candidates",
            description=(
                "Phase153A: build C1/C2 pivot target sequence tensors, run official tensor "
                "health checks, and audit label/payoff stability before any new training."
            ),
            fields=[
                CommandField("symbol", "Symbol", "XAUUSD"),
                CommandField(
                ...
                CommandField("candidates", "Candidates", "C1:stable:24:0.75:0.25:1.0;C2:dense:24:0.5:0.5:1.25"),
                CommandField("source_mode", "Source mode", "storage", kind="select", options=("storage", "yahoo", "files")),
                CommandField("yahoo_symbol", "Yahoo symbol", "GC=F"),
                CommandField("daily_range", "Daily range", "5y"),
                CommandField("hourly_range", "Hourly range", "730d"),
                CommandField("five_range", "5M range", "60d"),
                CommandField("hourly_timeframe", "Hourly timeframe", "1H"),
                CommandField("h4_timeframe", "4H timeframe", "4H"),
                CommandField("daily_timeframe", "Daily timeframe", "1D"),
                CommandField("daily_path", "Daily path", ""),
                CommandField("hourly_path", "Hourly path", ""),
                CommandField("h4_path", "4H path", ""),
                CommandField("five_path", "5M path", ""),
                CommandField("recent_window_bars", "Recent window bars", "48", kind="number"),
                CommandField("window_size", "Window size", "100", kind="number"),
                CommandField("sample_stride", "Sample stride", "1", kind="number"),
                CommandField("build_max_samples", "Build max samples", "0", kind="number"),
                CommandField("audit_max_samples", "Audit max samples", "24000", kind="number"),
                CommandField("include_source_5m_features", "Include source 5M features", "1", kind="select", options=("1", "0")),
                CommandField("source_5m_feature_path", "Source 5M feature path", ""),
                CommandField("max_features", "Max features", "0", kind="number"),
                CommandField("dtype", "Tensor dtype", "float16", kind="select", options=("float16", "float32")),
                CommandField("axis_normalization", "Axis normalization", "robust", kind="select", options=("robust", "standard", "none")),
                CommandField("feature_clip", "Feature clip", "8", kind="number"),
                CommandField("chunk_size", "Chunk size", "512", kind="number"),
                CommandField("max_tensor_mb", "Max tensor MB", "8192", kind="number"),
                CommandField("health_full_scan", "Health full scan", "1", kind="select", options=("1", "0")),
                CommandField("health_max_scan_samples", "Health max scan samples", "0", kind="number"),
                CommandField("train_frac", "Train fraction", "0.70", kind="number"),
                CommandField("val_frac", "Validation fraction", "0.15", kind="number"),
                CommandField("purge_gap", "Purge gap samples", "336", kind="number"),
                CommandField("psi_warning_threshold", "PSI warning threshold", "0.20", kind="number"),
                CommandField("skip_existing_tensors", "Skip existing tensors", "0", kind="select", options=("0", "1")),
                CommandField("output_dir", "Output dir", "run_logs/pivot_target_redesign_candidates"),
                CommandField("report_title", "Report title", "Phase153A pivot target redesign candidate build/audit"),
                CommandField("timeout_minutes", "Give up after (minutes)", "720", kind="number"),
            ],
            slow=True,
            group="AI",
        ...
        CommandDescriptor(
            kind=CommandKind.AUDIT_PIVOT_PAYOFF_TARGET_REDESIGN,
            label="Audit pivot payoff target redesign",
            description=(
                "Phase154A: audit first-hit/barrier trade-outcome pivot targets so we can "
                "find a payoff target that transfers before training another model."
            ),
            fields=[
                CommandField("symbol", "Symbol", "XAUUSD"),
                CommandField(
                ...
                CommandField(
                    "candidates",
                    "Payoff candidates",
                    "B1:balanced_24_rr1:24:0.35:0.75:0.75:0.0;B2:conservative_24_tp1_sl075:24:0.25:1.0:0.75:0.1;B3:dense_24_half_atr:24:0.50:0.50:0.50:0.0;B4:asym_24_tp1_sl05:24:0.35:1.0:0.50:0.1;B5:short_12_rr1:12:0.35:0.75:0.75:0.0;B6:wide_36_rr1:36:0.35:0.75:0.75:0.0",
                ),
                CommandField("max_samples", "Max sampled windows", "24000", kind="number"),
                CommandField("train_frac", "Train fraction", "0.70", kind="number"),
                CommandField("val_frac", "Validation fraction", "0.15", kind="number"),
                ...
                CommandField(
                    ...
                    "zero",
                    kind="select",
                    options=("zero", "close_r"),
                ),
                CommandField("psi_warning_threshold", "PSI warning threshold", "0.20", kind="number"),
                CommandField("min_train_actionable_rate", "Min train actionable", "0.02", kind="number"),
                CommandField("min_validation_actionable_rate", "Min validation actionable", "0.02", kind="number"),
                CommandField("min_test_actionable_rate", "Min test actionable", "0.02", kind="number"),
                CommandField("output_dir", "Output dir", "run_logs/pivot_payoff_target_redesign"),
                CommandField("report_title", "Report title", "Phase154A pivot payoff target redesign audit"),
                CommandField("timeout_minutes", "Give up after (minutes)", "240", kind="number"),
            ],
            slow=True,
            group="AI",
        ...
        registry.update(
            {
                ...
                CommandKind.INSPECT_HYBRID_TELEMETRY_TENSOR: (
                    accounts.inspect_hybrid_telemetry_tensor
                ),
                ...
                CommandKind.RUN_HYBRID_TENSOR_WALK_FORWARD_VALIDATION: (
                    accounts.run_hybrid_tensor_walk_forward_validation
                ),
                CommandKind.ANALYZE_TENSOR_FAILURE_REGIMES: (
                    accounts.analyze_tensor_failure_regimes
                ),
                CommandKind.BACKTEST_REGIME_FILTERED_HYBRID: (
                    accounts.backtest_regime_filtered_hybrid
                ),
                CommandKind.ANALYZE_TRADE_ANATOMY: accounts.analyze_trade_anatomy,
                CommandKind.BACKTEST_BRACKET_RECALIBRATION: (
                    accounts.backtest_bracket_recalibration
                ),
                CommandKind.AUDIT_CANDIDATE_DIRECTION_ENTRY: (
                    accounts.audit_candidate_direction_entry
                ),
                CommandKind.TRAIN_PIVOT_PATTERN_RECOGNITION: (
                    accounts.train_pivot_pattern_recognition
                ),
                CommandKind.BUILD_PIVOT_PATTERN_TENSOR: accounts.build_pivot_pattern_tensor,
                CommandKind.TRAIN_PIVOT_PATTERN_WAVENET: accounts.train_pivot_pattern_wavenet,
                CommandKind.BACKTEST_PIVOT_PATTERN_WAVENET: (
                    accounts.backtest_pivot_pattern_wavenet
                ),
                CommandKind.RUN_PIVOT_PATTERN_WAVENET_WALK_FORWARD: (
                    accounts.run_pivot_pattern_wavenet_walk_forward
                ),
                CommandKind.BUILD_PIVOT_PATTERN_IMAGE_TENSOR: (
                    accounts.build_pivot_pattern_image_tensor
                ),
                CommandKind.AUDIT_PIVOT_PATTERN_IMAGE_TENSOR: (
                    accounts.audit_pivot_pattern_image_tensor
                ),
                CommandKind.TRAIN_PIVOT_PATTERN_IMAGE_CNN: accounts.train_pivot_pattern_image_cnn,
                CommandKind.TRAIN_PIVOT_PATTERN_IMAGE_WAVENET: (
                    accounts.train_pivot_pattern_image_wavenet
                ),
                CommandKind.BUILD_PIVOT_PATTERN_SEQUENCE_TENSOR: (
                    accounts.build_pivot_pattern_sequence_tensor
                ),
                CommandKind.AUDIT_PIVOT_PATTERN_SEQUENCE_TENSOR: (
                    accounts.audit_pivot_pattern_sequence_tensor
                ),
                CommandKind.TRAIN_PIVOT_PATTERN_SEQUENCE_WAVENET: (
                ...
                CommandKind.AUDIT_PIVOT_SEQUENCE_RANGE_BRACKET_GEOMETRY: (
                    accounts.audit_pivot_sequence_range_bracket_geometry
                ),
                CommandKind.RUN_PIVOT_SEQUENCE_OPTION_B_SEED_TRANSFER: (
                    accounts.run_pivot_sequence_option_b_seed_transfer
                ),
                CommandKind.AUDIT_PIVOT_LABEL_TARGET_STABILITY: (
                    accounts.audit_pivot_label_target_stability
                ),
                CommandKind.RUN_PIVOT_TARGET_REDESIGN_CANDIDATE_AUDIT: (
                    accounts.run_pivot_target_redesign_candidate_audit
                ),
...
class AccountCommandHandlers(CommandHandlers):
    ...
    def inspect_hybrid_telemetry_tensor(self, command: Command) -> CommandResult:
        """Create the Phase127 tensor visual inspection HTML from GUI/Dashboard."""
        started = time.monotonic()
        symbol = command.text("symbol", "XAUUSD").strip().upper()
        dataset = command.text("dataset", "").strip().upper()
        available = stored_dataset_choices(self._storage_root)
        ...
        return self._run_script(
            ...
            [
                ...
                dataset,
                "--sample-indices",
                command.text("sample_indices", "first,middle,last").strip() or "first,middle,last",
                "--time-buckets",
                str(max(command.integer("time_buckets", 50), 5)),
                "--max-samples",
                str(max(command.integer("max_samples", 6), 1)),
                "--storage-root",
                str(self._storage_root),
                *tensor_args,
                *flat_args,
            ],
            f"Inspected Phase127 hybrid telemetry tensor on {symbol} {dataset}",
            started,
            timeout=max(command.integer("timeout_minutes", 60), 5) * 60,
        )
    ...
    def train_hybrid_telemetry_wavenet(self, command: Command) -> CommandResult:
        ...
        return self._run_script(
            ...
            [
                ...
                str(max(command.integer("max_samples", 0), 0)),
                "--loader-mode",
                command.text("loader_mode", "stream").strip().lower() or "stream",
                "--stream-chunk-size",
                str(max(command.integer("stream_chunk_size", 256), 1)),
                "--batch-size",
    ...
    def run_hybrid_tensor_walk_forward_validation(self, command: Command) -> CommandResult:
        """Run Phase133B tensor-model walk-forward validation from GUI/Dashboard."""
        started = time.monotonic()
        symbol = command.text("symbol", "XAUUSD").strip().upper()
        dataset = command.text("dataset", "").strip().upper()
        available = stored_dataset_choices(self._storage_root)
        ...
        return self._run_script(
            ...
            [
                ...
                dataset,
                "--model-family",
                command.text("model_family", "wavenet").strip().lower() or "wavenet",
                "--model-id",
                command.text("model_id", "gold_hybrid_tensor_walkforward_wavenet_5m").strip()
                or "gold_hybrid_tensor_walkforward_wavenet_5m",
                "--task",
                command.text("task", "multihead").strip().lower() or "multihead",
                "--candidate-only",
                "1" if command.text("candidate_only", "1").strip() != "0" else "0",
                ...
                str(max(command.integer("purge_gap_bars", 336), 0)),
                "--decision-modes",
                command.text("decision_modes", "score,both").strip() or "score,both",
                "--meta-thresholds",
                command.text("meta_thresholds", "0.55,0.60,0.65,0.70").strip()
                or "0.55,0.60,0.65,0.70",
                "--score-thresholds",
                command.text("score_thresholds", "0,0.05,0.10,0.20").strip() or "0,0.05,0.10,0.20",
                "--default-meta-threshold",
                str(command.number("default_meta_threshold", 0.55)),
                "--default-score-threshold",
                str(command.number("default_score_threshold", 0.0)),
                "--min-trades",
                str(max(command.integer("min_trades", 10), 0)),
                "--allow-no-trade",
                "1" if command.text("allow_no_trade", "0").strip() == "1" else "0",
                "--min-validation-score",
                str(command.number("min_validation_score", -1e18)),
                "--min-validation-profit-factor",
                str(max(command.number("min_validation_profit_factor", 0.0), 0.0)),
                "--max-validation-drawdown",
                str(command.number("max_validation_drawdown", -1.0)),
                "--min-train-samples",
                str(max(command.integer("min_train_samples", 200), 20)),
                "--score-metric",
                command.text("score_metric", "total_pnl").strip() or "total_pnl",
                "--max-folds",
                str(max(command.integer("max_folds", 0), 0)),
                "--max-train-samples",
                str(max(command.integer("max_train_samples", 0), 0)),
                "--batch-size",
                str(max(command.integer("batch_size", 64), 1)),
                "--epochs",
                str(max(command.integer("epochs", 120), 1)),
                "--learning-rate",
                str(max(command.number("learning_rate", 0.001), 1e-8)),
                "--filters",
                str(max(command.integer("filters", 48), 1)),
                ...
                str(max(command.integer("early_stopping_patience", 8), 0)),
                "--verbose",
                str(min(max(command.integer("verbose", 2), 0), 2)),
                "--initial-capital",
                str(max(command.number("initial_capital", 100.0), 0.0)),
                "--units",
                str(max(command.number("units", 0.1), 0.0)),
            ...
            f"Ran Phase133B tensor walk-forward validation on {symbol} {dataset}",
            started,
            timeout=max(command.integer("timeout_minutes", 480), 5) * 60,
        )

    def analyze_tensor_failure_regimes(self, command: Command) -> CommandResult:
        """Run Phase136A tensor failure/regime diagnostics from GUI/Dashboard."""
        started = time.monotonic()
        symbol = command.text("symbol", "XAUUSD").strip().upper()
        dataset = command.text("dataset", "").strip().upper()
        available = stored_dataset_choices(self._storage_root)
        ...
        return self._run_script(
            ...
            [
                ...
                or "run_logs/hybrid_tensor_walk_forward_validation/latest.json",
                "--candidate-only",
                "1" if command.text("candidate_only", "1").strip() != "0" else "0",
                "--min-group-rows",
                str(max(command.integer("min_group_rows", 20), 1)),
                "--top-n",
                str(max(command.integer("top_n", 12), 1)),
                "--quantile-bins",
                str(max(command.integer("quantile_bins", 5), 2)),
                "--storage-root",
                str(self._storage_root),
                *flat_args,
            ],
            f"Analyzed Phase136A tensor failure regimes on {symbol} {dataset}",
            started,
            timeout=max(command.integer("timeout_minutes", 60), 5) * 60,
        )

    def backtest_regime_filtered_hybrid(self, command: Command) -> CommandResult:
        """Run Phase137A side-aware regime-filtered hybrid replay."""
        started = time.monotonic()
        symbol = command.text("symbol", "XAUUSD").strip().upper()
        dataset = command.text("dataset", "").strip().upper()
        available = stored_dataset_choices(self._storage_root)
        ...
        return self._run_script(
            ...
            [
                ...
                command.text("end_month", "").strip(),
                "--allowed-sides",
                command.text("allowed_sides", "BUY,SELL").strip() or "BUY,SELL",
                "--allowed-hours",
                command.text("allowed_hours", "").strip(),
                "--buy-allowed-hours",
                command.text("buy_allowed_hours", "").strip(),
                "--sell-allowed-hours",
                command.text("sell_allowed_hours", "").strip(),
                "--min-candidate-confidence",
                str(max(command.number("min_candidate_confidence", 0.0), 0.0)),
                "--buy-min-confidence",
                str(command.number("buy_min_confidence", -1.0)),
                "--sell-min-confidence",
                str(command.number("sell_min_confidence", -1.0)),
                "--min-reward-risk",
                str(max(command.number("min_reward_risk", 0.0), 0.0)),
                "--buy-min-reward-risk",
                str(command.number("buy_min_reward_risk", -1.0)),
                "--sell-min-reward-risk",
                str(command.number("sell_min_reward_risk", -1.0)),
                "--max-sl-distance",
                str(command.number("max_sl_distance", -1.0)),
                "--buy-max-sl-distance",
                str(command.number("buy_max_sl_distance", -1.0)),
                "--sell-max-sl-distance",
                str(command.number("sell_max_sl_distance", -1.0)),
                "--min-side-4h-room",
                str(max(command.number("min_side_4h_room", 0.0), 0.0)),
                "--buy-min-side-4h-room",
                str(command.number("buy_min_side_4h_room", -1.0)),
                "--sell-min-side-4h-room",
                str(command.number("sell_min_side_4h_room", -1.0)),
                "--min-side-1d-room",
                str(max(command.number("min_side_1d_room", 0.0), 0.0)),
                "--buy-min-side-1d-room",
                str(command.number("buy_min_side_1d_room", -1.0)),
                "--sell-min-side-1d-room",
                str(command.number("sell_min_side_1d_room", -1.0)),
                "--min-range-4h-width",
                str(command.number("min_range_4h_width", -1.0)),
                "--max-range-4h-width",
                str(command.number("max_range_4h_width", -1.0)),
                "--min-range-1d-width",
                str(command.number("min_range_1d_width", -1.0)),
                "--max-range-1d-width",
                str(command.number("max_range_1d_width", -1.0)),
                "--block-specialist-conflict",
                "1" if command.text("block_specialist_conflict", "0").strip() == "1" else "0",
                "--max-booster-entropy",
                str(command.number("max_booster_entropy", -1.0)),
                "--min-booster-action-margin",
                str(max(command.number("min_booster_action_margin", 0.0), 0.0)),
                "--min-specialist-max-prob",
                str(max(command.number("min_specialist_max_prob", 0.0), 0.0)),
                "--min-trades",
                str(max(command.integer("min_trades", 10), 0)),
                "--score-metric",
                command.text("score_metric", "total_pnl").strip() or "total_pnl",
                ...
                str(max(command.number("units", 0.1), 0.0)),
                "--report-title",
                command.text("report_title", "Regime-filtered hybrid replay").strip()
                or "Regime-filtered hybrid replay",
                "--storage-root",
                str(self._storage_root),
                *flat_args,
            ],
            f"Backtested Phase137A regime-filtered hybrid on {symbol} {dataset}",
            started,
            timeout=max(command.integer("timeout_minutes", 120), 5) * 60,
        )

    def analyze_trade_anatomy(self, command: Command) -> CommandResult:
        """Run Phase138A TP/SL and trade anatomy diagnostics from GUI/Dashboard."""
        started = time.monotonic()
        symbol = command.text("symbol", "XAUUSD").strip().upper()
        dataset = command.text("dataset", "").strip().upper()
        available = stored_dataset_choices(self._storage_root)
        ...
        return self._run_script(
            ...
            [
                ...
                str(max(command.integer("quantile_bins", 5), 2)),
                "--top-n",
                str(max(command.integer("top_n", 15), 1)),
                "--storage-root",
                str(self._storage_root),
                *flat_args,
            ],
            f"Analyzed Phase138A trade anatomy on {symbol} {dataset}",
            started,
            timeout=max(command.integer("timeout_minutes", 60), 5) * 60,
        )

    def backtest_bracket_recalibration(self, command: Command) -> CommandResult:
        """Run Phase139A TP/SL bracket recalibration replay from GUI/Dashboard."""
        started = time.monotonic()
        symbol = command.text("symbol", "XAUUSD").strip().upper()
        dataset = command.text("dataset", "").strip().upper()
        available = stored_dataset_choices(self._storage_root)
        ...
        return self._run_script(
            ...
            [
                ...
                command.text("allowed_sides", "BUY,SELL").strip() or "BUY,SELL",
                "--max-tp-distances",
                command.text("max_tp_distances", "original,12,14,16,18").strip()
                or "original,12,14,16,18",
                "--max-sl-distances",
                command.text("max_sl_distances", "original,20,23.56,30").strip()
                or "original,20,23.56,30",
                "--max-reward-risks",
                command.text("max_reward_risks", "original,1.0,1.5,2.0").strip()
                or "original,1.0,1.5,2.0",
                "--buy-max-tp-distance",
                str(command.number("buy_max_tp_distance", -1.0)),
                "--buy-max-sl-distance",
                str(command.number("buy_max_sl_distance", -1.0)),
                "--buy-max-reward-risk",
                str(command.number("buy_max_reward_risk", -1.0)),
                "--sell-max-tp-distance",
                str(command.number("sell_max_tp_distance", -1.0)),
                "--sell-max-sl-distance",
                str(command.number("sell_max_sl_distance", -1.0)),
                "--sell-max-reward-risk",
                str(command.number("sell_max_reward_risk", -1.0)),
                "--min-tp-distance",
                str(max(command.number("min_tp_distance", 0.1), 0.0)),
                "--min-sl-distance",
                str(max(command.number("min_sl_distance", 0.1), 0.0)),
                "--max-hold-bars",
                str(max(command.integer("max_hold_bars", 48), 1)),
                "--spread-mode",
                command.text("spread_mode", "pct").strip().lower() or "pct",
                ...
                command.text("score_metric", "total_pnl").strip() or "total_pnl",
                "--max-policies",
                str(max(command.integer("max_policies", 0), 0)),
                "--initial-capital",
                str(max(command.number("initial_capital", 100.0), 0.0)),
                "--units",
                str(max(command.number("units", 0.1), 0.0)),
                "--report-title",
                command.text("report_title", "TP/SL bracket recalibration replay").strip()
                or "TP/SL bracket recalibration replay",
                "--storage-root",
                str(self._storage_root),
                *flat_args,
            ],
            f"Backtested Phase139A bracket recalibration on {symbol} {dataset}",
            started,
            timeout=max(command.integer("timeout_minutes", 180), 5) * 60,
        )

    def audit_candidate_direction_entry(self, command: Command) -> CommandResult:
        """Run Phase140A candidate direction / delayed-entry audit from GUI."""
        started = time.monotonic()
        symbol = command.text("symbol", "XAUUSD").strip().upper()
        dataset = command.text("dataset", "").strip().upper()
        available = stored_dataset_choices(self._storage_root)
        ...
        return self._run_script(
            ...
            [
                ...
                command.text("allowed_sides", "BUY,SELL").strip() or "BUY,SELL",
                "--entry-delays",
                command.text("entry_delays", "0,1,2,3").strip() or "0,1,2,3",
                "--side-modes",
                command.text("side_modes", "original,flipped").strip() or "original,flipped",
                "--execution-modes",
                command.text("execution_modes", "independent,chronological").strip()
                or "independent,chronological",
                "--eval-frac",
                str(command.number("eval_frac", 1.0)),
                "--max-windows",
                str(max(command.integer("max_windows", 0), 0)),
                ...
                str(max(command.integer("min_trades", 10), 0)),
                "--min-group-trades",
                str(max(command.integer("min_group_trades", 5), 1)),
                "--score-metric",
                command.text("score_metric", "total_pnl").strip() or "total_pnl",
                "--initial-capital",
                str(max(command.number("initial_capital", 100.0), 0.0)),
                ...
                str(max(command.number("units", 0.1), 0.0)),
                "--report-title",
                command.text("report_title", "Candidate direction / entry audit").strip()
                or "Candidate direction / entry audit",
                "--storage-root",
                str(self._storage_root),
                *flat_args,
            ],
            f"Audited Phase140A candidate direction/entry on {symbol} {dataset}",
            started,
            timeout=max(command.integer("timeout_minutes", 180), 5) * 60,
        )

    def train_pivot_pattern_recognition(self, command: Command) -> CommandResult:
        """Run Phase141A pivot/top-bottom pattern-recognition training and WF test."""
        started = time.monotonic()
        ...
        return self._run_script(
            ...
            [
                ...
                "--source-mode",
                command.text("source_mode", "storage").strip().lower() or "storage",
                "--symbol",
                symbol,
                "--yahoo-symbol",
                command.text("yahoo_symbol", "GC=F").strip() or "GC=F",
                "--daily-range",
                command.text("daily_range", "5y").strip() or "5y",
                "--hourly-range",
                command.text("hourly_range", "730d").strip() or "730d",
                "--five-range",
                command.text("five_range", "60d").strip() or "60d",
                "--five-timeframe",
                command.text("five_timeframe", "5M").strip().upper() or "5M",
                "--hourly-timeframe",
                command.text("hourly_timeframe", "1H").strip().upper() or "1H",
                "--h4-timeframe",
                command.text("h4_timeframe", "4H").strip().upper() or "4H",
                "--daily-timeframe",
                command.text("daily_timeframe", "1D").strip().upper() or "1D",
                "--daily-path",
                command.text("daily_path", "").strip(),
                "--hourly-path",
                command.text("hourly_path", "").strip(),
                "--h4-path",
                command.text("h4_path", "").strip(),
                "--five-path",
                command.text("five_path", "").strip(),
                "--model-id",
                command.text("model_id", "gold_pivot_pattern_recognition_5m").strip()
                or "gold_pivot_pattern_recognition_5m",
                "--model-kind",
                command.text("model_kind", "auto").strip().lower() or "auto",
                "--lookahead-bars",
                str(max(command.integer("lookahead_bars", 48), 1)),
                "--pivot-move-atr",
                str(max(command.number("pivot_move_atr", 0.75), 0.01)),
                "--pivot-zone-atr",
                str(max(command.number("pivot_zone_atr", 0.35), 0.01)),
                "--recent-window-bars",
                str(max(command.integer("recent_window_bars", 48), 1)),
                "--min-direction-edge",
                str(max(command.number("min_direction_edge", 1.10), 1.0)),
                "--max-features",
                str(max(command.integer("max_features", 38), 1)),
                "--train-days-min",
                str(max(command.integer("train_days_min", 20), 1)),
                "--validation-days",
                str(max(command.integer("validation_days", 5), 1)),
                "--test-days",
                str(max(command.integer("test_days", 5), 1)),
                "--purge-hours",
                str(max(command.number("purge_hours", 4.0), 0.0)),
                "--buy-thresholds",
                command.text("buy_thresholds", "0.45,0.55,0.65").strip() or "0.45,0.55,0.65",
                "--sell-thresholds",
                command.text("sell_thresholds", "0.45,0.55,0.65").strip() or "0.45,0.55,0.65",
                "--margins",
                command.text("margins", "0,0.05").strip() or "0,0.05",
                "--tp-multipliers",
                command.text("tp_multipliers", "0.75,1.0,1.25").strip() or "0.75,1.0,1.25",
                "--sl-multipliers",
                command.text("sl_multipliers", "0.50,0.75").strip() or "0.50,0.75",
                "--hold-bars",
                command.text("hold_bars", "24,48").strip() or "24,48",
                "--min-validation-trades",
                str(max(command.integer("min_validation_trades", 5), 0)),
                "--score-metric",
                command.text("score_metric", "net_profit").strip() or "net_profit",
                "--spread-mode",
                command.text("spread_mode", "pct").strip().lower() or "pct",
                "--spread-value",
                str(max(command.number("spread_value", 0.06), 0.0)),
                ...
                "--save-model",
                "1" if command.text("save_model", "1").strip() != "0" else "0",
                "--save-data",
                "1" if command.text("save_data", "1").strip() != "0" else "0",
                "--report-title",
                command.text("report_title", "Pivot pattern recognition walk-forward").strip()
                or "Pivot pattern recognition walk-forward",
                "--storage-root",
                str(self._storage_root),
            ],
            f"Trained Phase141A pivot pattern recognition for {symbol}",
            started,
            timeout=max(command.integer("timeout_minutes", 240), 5) * 60,
        )

    def build_pivot_pattern_tensor(self, command: Command) -> CommandResult:
        """Build Phase142A 3D pivot-pattern tensor from project storage/Yahoo/files."""
        started = time.monotonic()
        symbol = command.text("symbol", "XAUUSD").strip().upper() or "XAUUSD"
        return self._run_script(
            command,
            [
                ...
                str(max(command.number("min_direction_edge", 1.10), 1.0)),
                "--tensor-window",
                str(max(command.integer("tensor_window", 288), 2)),
                "--sample-stride",
                str(max(command.integer("sample_stride", 1), 1)),
                "--max-samples",
                str(max(command.integer("max_samples", 0), 0)),
                "--dtype",
                command.text("dtype", "float16").strip() or "float16",
                "--max-tensor-mb",
                str(max(command.number("max_tensor_mb", 8192.0), 1.0)),
                "--axis-normalization",
                command.text("axis_normalization", "robust").strip().lower() or "robust",
                "--feature-clip",
                str(max(command.number("feature_clip", 8.0), 0.0)),
                "--interaction-clip",
                str(max(command.number("interaction_clip", 32.0), 0.0)),
                "--output-name",
                command.text("output_name", "pivot_pattern_tensor_latest").strip()
                or "pivot_pattern_tensor_latest",
                "--storage-root",
                str(self._storage_root),
            ],
            f"Built Phase142A pivot pattern tensor for {symbol}",
            started,
            timeout=max(command.integer("timeout_minutes", 180), 5) * 60,
        )

    def train_pivot_pattern_wavenet(self, command: Command) -> CommandResult:
        """Train Phase142A Keras WaveNet on pivot-pattern tensor."""
        started = time.monotonic()
        symbol = command.text("symbol", "XAUUSD").strip().upper() or "XAUUSD"
        ...
        return self._run_script(
            ...
            [
                ...
                dataset,
                "--model-id",
                command.text("model_id", "gold_pivot_pattern_wavenet_5m").strip()
                or "gold_pivot_pattern_wavenet_5m",
                "--train-frac",
                str(command.number("train_frac", 0.70)),
                "--val-frac",
                str(command.number("val_frac", 0.15)),
                ...
                str(max(command.integer("epochs", 120), 1)),
                "--learning-rate",
                str(max(command.number("learning_rate", 0.001), 1e-9)),
                "--filters",
                str(max(command.integer("filters", 48), 1)),
                "--kernel-size",
                str(max(command.integer("kernel_size", 3), 2)),
                "--n-layers",
                str(max(command.integer("n_layers", 6), 1)),
                "--n-blocks",
                str(max(command.integer("n_blocks", 2), 1)),
                "--dense-units",
                str(max(command.integer("dense_units", 96), 8)),
                "--dropout",
                str(max(command.number("dropout", 0.20), 0.0)),
                "--action-loss-weight",
                str(max(command.number("action_loss_weight", 1.0), 0.0)),
                "--top-loss-weight",
                str(max(command.number("top_loss_weight", 0.5), 0.0)),
                "--bottom-loss-weight",
                str(max(command.number("bottom_loss_weight", 0.5), 0.0)),
                "--r-loss-weight",
                str(max(command.number("r_loss_weight", 0.5), 0.0)),
                "--class-weight",
                command.text("class_weight", "auto").strip() or "auto",
                "--buy-threshold",
                str(command.number("buy_threshold", 0.55)),
                "--sell-threshold",
                str(command.number("sell_threshold", 0.55)),
                "--min-margin",
                str(max(command.number("min_margin", 0.05), 0.0)),
                "--min-buy-r",
                str(command.number("min_buy_r", 0.0)),
                "--min-sell-r",
                str(command.number("min_sell_r", 0.0)),
                "--monitor-metric",
                command.text("monitor_metric", "auto").strip() or "auto",
                "--early-stopping-patience",
                str(max(command.integer("early_stopping_patience", 12), 1)),
                "--checkpoint-each-epoch",
                "1" if command.text("checkpoint_each_epoch", "1").strip() != "0" else "0",
                "--batch-log-every",
                str(max(command.integer("batch_log_every", 10), 0)),
                "--batch-log-file",
                command.text("batch_log_file", "").strip(),
                "--save-model",
                "1" if command.text("save_model", "1").strip() != "0" else "0",
                "--report-title",
                command.text("report_title", "Pivot WaveNet training").strip()
                or "Pivot WaveNet training",
                "--storage-root",
                str(self._storage_root),
                *tensor_args,
            ],
            f"Trained Phase142A pivot WaveNet for {symbol} {dataset}",
            started,
            timeout=max(command.integer("timeout_minutes", 480), 5) * 60,
        )

    def backtest_pivot_pattern_wavenet(self, command: Command) -> CommandResult:
        """Backtest Phase142A Keras WaveNet on pivot-pattern tensor."""
        started = time.monotonic()
        symbol = command.text("symbol", "XAUUSD").strip().upper() or "XAUUSD"
        dataset = command.text("dataset", "5M").strip().upper() or "5M"
        tensor_path = command.text("tensor_path", "").strip()
        flat_path = command.text("flat_path", "").strip()
        model_path = command.text("model_path", "").strip()
        record_path = command.text("record_path", "").strip()
        extra_args = []
        if tensor_path:
            extra_args.extend(["--tensor-path", tensor_path])
        if flat_path:
            extra_args.extend(["--flat-path", flat_path])
        if model_path and record_path:
            extra_args.extend(["--model-path", model_path, "--record-path", record_path])
        return self._run_script(
            command,
            [
                ...
                str(command.number("min_margin", -1.0)),
                "--top-threshold",
                str(command.number("top_threshold", 0.50)),
                "--bottom-threshold",
                str(command.number("bottom_threshold", 0.50)),
                "--min-buy-r",
                ...
                str(max(command.number("risk_per_trade", 0.01), 0.0)),
                "--report-title",
                command.text("report_title", "Pivot WaveNet backtest").strip()
                or "Pivot WaveNet backtest",
                "--storage-root",
                str(self._storage_root),
                *extra_args,
            ],
            f"Backtested Phase142A pivot WaveNet for {symbol} {dataset}",
            started,
            timeout=max(command.integer("timeout_minutes", 180), 5) * 60,
        )

    def run_pivot_pattern_wavenet_walk_forward(self, command: Command) -> CommandResult:
        """Run Phase142B fold-by-fold Keras WaveNet walk-forward validation."""
        started = time.monotonic()
        symbol = command.text("symbol", "XAUUSD").strip().upper() or "XAUUSD"
        dataset = command.text("dataset", "5M").strip().upper() or "5M"
        tensor_path = command.text("tensor_path", "").strip()
        ...
        return self._run_script(
            ...
            [
                ...
                dataset,
                "--model-id",
                command.text("model_id", "gold_pivot_pattern_wavenet_wf_5m").strip()
                or "gold_pivot_pattern_wavenet_wf_5m",
                "--train-days-min",
                str(max(command.integer("train_days_min", 120), 1)),
                "--validation-days",
                str(max(command.integer("validation_days", 20), 1)),
                "--test-days",
                str(max(command.integer("test_days", 20), 1)),
                "--purge-hours",
                str(max(command.number("purge_hours", 4.0), 0.0)),
                "--max-folds",
                str(max(command.integer("max_folds", 0), 0)),
                ...
                str(max(command.integer("batch_size", 64), 1)),
                "--epochs",
                str(max(command.integer("epochs", 80), 1)),
                "--learning-rate",
                str(max(command.number("learning_rate", 0.001), 1e-9)),
                "--filters",
                str(max(command.integer("filters", 48), 1)),
                ...
                command.text("class_weight", "auto").strip() or "auto",
                "--early-stopping-patience",
                str(max(command.integer("early_stopping_patience", 10), 1)),
                "--buy-thresholds",
                command.text("buy_thresholds", "0.45,0.55,0.65").strip() or "0.45,0.55,0.65",
                "--sell-thresholds",
                command.text("sell_thresholds", "0.45,0.55,0.65").strip() or "0.45,0.55,0.65",
                ...
                command.text("margins", "0,0.05").strip() or "0,0.05",
                "--top-thresholds",
                command.text("top_thresholds", "0.45,0.55,0.65").strip() or "0.45,0.55,0.65",
                "--bottom-thresholds",
                command.text("bottom_thresholds", "0.45,0.55,0.65").strip() or "0.45,0.55,0.65",
                "--min-r-values",
                command.text("min_r_values", "-0.25,0,0.25").strip() or "-0.25,0,0.25",
                "--tp-multipliers",
                command.text("tp_multipliers", "0.75,1.0,1.25").strip() or "0.75,1.0,1.25",
                "--sl-multipliers",
                command.text("sl_multipliers", "0.50,0.75").strip() or "0.50,0.75",
                ...
                command.text("hold_bars", "24,48").strip() or "24,48",
                "--min-validation-trades",
                str(max(command.integer("min_validation_trades", 10), 0)),
                "--score-metric",
                command.text("score_metric", "drawdown_adjusted").strip() or "drawdown_adjusted",
                "--spread-mode",
                command.text("spread_mode", "pct").strip().lower() or "pct",
                "--spread-value",
                str(max(command.number("spread_value", 0.06), 0.0)),
                ...
                str(max(command.number("risk_per_trade", 0.01), 0.0)),
                "--report-title",
                command.text("report_title", "Pivot WaveNet walk-forward validation").strip()
                or "Pivot WaveNet walk-forward validation",
                "--storage-root",
                str(self._storage_root),
                *extra_args,
            ],
            f"Ran Phase142B pivot WaveNet walk-forward for {symbol} {dataset}",
            started,
            timeout=max(command.integer("timeout_minutes", 720), 5) * 60,
        )

    def build_pivot_pattern_image_tensor(self, command: Command) -> CommandResult:
        """Build Phase143A 4D image tensor for Conv2D/Conv3D."""
        started = time.monotonic()
        symbol = command.text("symbol", "XAUUSD").strip().upper() or "XAUUSD"
        return self._run_script(
            command,
            [
                ...
                str(max(command.number("min_direction_edge", 1.10), 1.0)),
                "--window-size",
                str(max(command.integer("window_size", 100), 2)),
                "--sample-stride",
                str(max(command.integer("sample_stride", 1), 1)),
                "--max-samples",
                str(max(command.integer("max_samples", 0), 0)),
                "--max-5m-features",
                str(max(command.integer("max_5m_features", 24), 1)),
                "--max-htf-features",
                str(max(command.integer("max_htf_features", 16), 1)),
                "--dtype",
                command.text("dtype", "float16").strip() or "float16",
                "--chunk-size",
                str(max(command.integer("chunk_size", 256), 1)),
                "--max-tensor-mb",
                str(max(command.number("max_tensor_mb", 8192.0), 1.0)),
                "--axis-normalization",
                command.text("axis_normalization", "robust").strip().lower() or "robust",
                ...
                str(max(command.number("interaction_clip", 32.0), 0.0)),
                "--output-name",
                command.text("output_name", "pivot_pattern_image_tensor_latest").strip()
                or "pivot_pattern_image_tensor_latest",
                "--storage-root",
                str(self._storage_root),
            ],
            f"Built Phase143A 4D pivot image tensor for {symbol}",
            started,
            timeout=max(command.integer("timeout_minutes", 240), 5) * 60,
        )

    def audit_pivot_pattern_image_tensor(self, command: Command) -> CommandResult:
        """Audit Phase143A 4D image tensor health before CNN training."""
        started = time.monotonic()
        symbol = command.text("symbol", "XAUUSD").strip().upper() or "XAUUSD"
        dataset = command.text("dataset", "5M").strip().upper() or "5M"
        tensor_path = command.text("tensor_path", "").strip()
        ...
        return self._run_script(
            ...
            [
                ...
                or "run_logs/pivot_pattern_image_tensor/latest.json",
                "--chunk-size",
                str(max(command.integer("chunk_size", 256), 1)),
                "--full-scan",
                ...
                (
                    "1"
                    if command.text("fail_on_reported_feature_nonfinite", "0").strip() == "1"
                    else "0"
                ),
                "--storage-root",
                str(self._storage_root),
            ...
            f"Audited Phase143A pivot image tensor health for {symbol} {dataset}",
            started,
            timeout=max(command.integer("timeout_minutes", 180), 5) * 60,
        )

    def train_pivot_pattern_image_cnn(self, command: Command) -> CommandResult:
        """Train Phase143A Conv2D/Conv3D model on 4D pivot image tensor."""
        started = time.monotonic()
        symbol = command.text("symbol", "XAUUSD").strip().upper() or "XAUUSD"
        dataset = command.text("dataset", "5M").strip().upper() or "5M"
        tensor_path = command.text("tensor_path", "").strip()
        ...
        return self._run_script(
            ...
            [
                ...
                dataset,
                "--model-id",
                command.text("model_id", "gold_pivot_pattern_image_cnn_5m").strip()
                or "gold_pivot_pattern_image_cnn_5m",
                "--model-kind",
                command.text("model_kind", "conv2d").strip().lower() or "conv2d",
                "--train-frac",
                str(command.number("train_frac", 0.70)),
                "--val-frac",
                str(command.number("val_frac", 0.15)),
                ...
                str(max(command.number("learning_rate", 0.001), 1e-9)),
                "--filters",
                str(max(command.integer("filters", 32), 1)),
                "--kernel-size",
                str(max(command.integer("kernel_size", 3), 2)),
                "--dense-units",
                str(max(command.integer("dense_units", 96), 8)),
                ...
                "1" if command.text("save_model", "1").strip() != "0" else "0",
                "--report-title",
                command.text("report_title", "Pivot image CNN training").strip()
                or "Pivot image CNN training",
                "--storage-root",
                str(self._storage_root),
                *extra_args,
            ],
            f"Trained Phase143A pivot image CNN for {symbol} {dataset}",
            started,
            timeout=max(command.integer("timeout_minutes", 480), 5) * 60,
        )

    def train_pivot_pattern_image_wavenet(self, command: Command) -> CommandResult:
        """Train Phase144A advanced 4D pivot image WaveNet."""
        started = time.monotonic()
        symbol = command.text("symbol", "XAUUSD").strip().upper() or "XAUUSD"
        dataset = command.text("dataset", "5M").strip().upper() or "5M"
        tensor_path = command.text("tensor_path", "").strip()
        ...
        return self._run_script(
            ...
            [
                ...
                dataset,
                "--model-id",
                command.text("model_id", "gold_pivot_pattern_image_wavenet_5m").strip()
                or "gold_pivot_pattern_image_wavenet_5m",
                "--train-frac",
                str(command.number("train_frac", 0.70)),
                "--val-frac",
                str(command.number("val_frac", 0.15)),
                ...
                str(max(command.integer("stream_chunk_size", 256), 1)),
                "--batch-size",
                str(max(command.integer("batch_size", 16), 1)),
                "--epochs",
                str(max(command.integer("epochs", 80), 1)),
                "--learning-rate",
                str(max(command.number("learning_rate", 0.0005), 1e-9)),
                "--spatial-filters",
                str(max(command.integer("spatial_filters", 32), 1)),
                "--branch-filters",
                str(max(command.integer("branch_filters", 32), 1)),
                "--temporal-filters",
                str(max(command.integer("temporal_filters", 64), 1)),
                "--spatial-kernels",
                command.text("spatial_kernels", "1,3,5").strip() or "1,3,5",
                "--temporal-kernels",
                command.text("temporal_kernels", "3,5,9").strip() or "3,5,9",
                "--dilations",
                command.text("dilations", "1,2,4,8,16,32").strip() or "1,2,4,8,16,32",
                "--residual-blocks",
                str(max(command.integer("residual_blocks", 2), 1)),
                "--attention-heads",
                str(max(command.integer("attention_heads", 4), 1)),
                "--attention-key-dim",
                str(max(command.integer("attention_key_dim", 16), 1)),
                "--se-ratio",
                str(max(command.integer("se_ratio", 8), 1)),
                "--dense-units",
                str(max(command.integer("dense_units", 128), 8)),
                "--dropout",
                str(max(command.number("dropout", 0.25), 0.0)),
                "--activation",
                command.text("activation", "tanh").strip().lower() or "tanh",
                "--action-loss-weight",
                str(max(command.number("action_loss_weight", 1.0), 0.0)),
                "--top-loss-weight",
                str(max(command.number("top_loss_weight", 0.5), 0.0)),
                ...
                str(command.number("min_sell_r", -999.0)),
                "--early-stopping-patience",
                str(max(command.integer("early_stopping_patience", 8), 1)),
                "--checkpoint-each-epoch",
                "1" if command.text("checkpoint_each_epoch", "1").strip() != "0" else "0",
                "--batch-log-every",
                str(max(command.integer("batch_log_every", 10), 0)),
                ...
                "1" if command.text("save_model", "1").strip() != "0" else "0",
                "--report-title",
                command.text("report_title", "Advanced Pivot Image WaveNet training").strip()
                or "Advanced Pivot Image WaveNet training",
                "--storage-root",
                str(self._storage_root),
                *extra_args,
            ],
            f"Trained Phase144A advanced pivot image WaveNet for {symbol} {dataset}",
            started,
            timeout=max(command.integer("timeout_minutes", 720), 5) * 60,
        )

    def build_pivot_pattern_sequence_tensor(self, command: Command) -> CommandResult:
        """Build Phase145A [samples, window, features] sequence tensor."""
        started = time.monotonic()
        symbol = command.text("symbol", "XAUUSD").strip().upper() or "XAUUSD"
        return self._run_script(
            command,
            [
                ...
                str(max(command.integer("max_samples", 0), 0)),
                "--include-source-5m-features",
                "1" if command.text("include_source_5m_features", "1").strip() != "0" else "0",
                "--source-5m-feature-path",
                command.text("source_5m_feature_path", "").strip(),
                "--max-features",
                str(max(command.integer("max_features", 0), 0)),
                "--dtype",
                command.text("dtype", "float16").strip() or "float16",
                "--axis-normalization",
                command.text("axis_normalization", "robust").strip().lower() or "robust",
                ...
                str(max(command.number("feature_clip", 8.0), 0.0)),
                "--chunk-size",
                str(max(command.integer("chunk_size", 512), 1)),
                "--max-tensor-mb",
                str(max(command.number("max_tensor_mb", 8192.0), 1.0)),
                "--output-name",
                command.text("output_name", "pivot_pattern_sequence_tensor_latest").strip()
                or "pivot_pattern_sequence_tensor_latest",
                "--storage-root",
                str(self._storage_root),
            ],
            f"Built Phase145A pivot sequence tensor for {symbol}",
            started,
            timeout=max(command.integer("timeout_minutes", 180), 5) * 60,
        )

    def audit_pivot_pattern_sequence_tensor(self, command: Command) -> CommandResult:
        """Audit Phase145A [samples, window, features] sequence tensor health."""
        started = time.monotonic()
        symbol = command.text("symbol", "XAUUSD").strip().upper() or "XAUUSD"
        dataset = command.text("dataset", "5M").strip().upper() or "5M"
        tensor_path = command.text("tensor_path", "").strip()
        ...
        return self._run_script(
            ...
            [
                ...
                or "run_logs\\pivot_pattern_sequence_tensor\\latest.json",
                "--chunk-size",
                str(max(command.integer("chunk_size", 512), 1)),
                "--full-scan",
                "1" if command.text("full_scan", "1").strip() != "0" else "0",
                ...
                "--fail-on-reported-feature-nonfinite",
                "1"
                if command.text("fail_on_reported_feature_nonfinite", "1").strip() != "0"
                else "0",
                "--storage-root",
                str(self._storage_root),
                "--output-dir",
                "run_logs\\pivot_pattern_sequence_tensor_health",
                *extra_args,
            ],
            f"Audited Phase145A pivot sequence tensor health for {symbol} {dataset}",
            started,
            timeout=max(command.integer("timeout_minutes", 240), 5) * 60,
        )

    def train_pivot_pattern_sequence_wavenet(self, command: Command) -> CommandResult:
        """Train Phase145A advanced sequence Conv1D/WaveNet."""
        started = time.monotonic()
        symbol = command.text("symbol", "XAUUSD").strip().upper() or "XAUUSD"
        dataset = command.text("dataset", "5M").strip().upper() or "5M"
        tensor_path = command.text("tensor_path", "").strip()
        ...
        return self._run_script(
            ...
            [
                ...
                str(max(command.number("learning_rate", 0.0005), 1e-9)),
                "--random-seed",
                str(max(command.integer("random_seed", 20260919), 0)),
                "--branch-filters",
                str(max(command.integer("branch_filters", 48), 1)),
                "--temporal-filters",
                str(max(command.integer("temporal_filters", 96), 1)),
                "--temporal-kernels",
                command.text("temporal_kernels", "3,5,9").strip() or "3,5,9",
                "--dilations",
                command.text("dilations", "1,2,4,8,16,32").strip() or "1,2,4,8,16,32",
                ...
                "1" if command.text("save_model", "1").strip() != "0" else "0",
                "--report-title",
                command.text("report_title", "Advanced Pivot Sequence WaveNet training").strip()
                or "Advanced Pivot Sequence WaveNet training",
                "--storage-root",
                str(self._storage_root),
                *extra_args,
            ],
    ...
    def train_pivot_pattern_sequence_wavenet_option_b(self, command: Command) -> CommandResult:
        """Train Phase147A Option B grouped-input sequence WaveNet."""
        started = time.monotonic()
        symbol = command.text("symbol", "XAUUSD").strip().upper() or "XAUUSD"
        dataset = command.text("dataset", "5M").strip().upper() or "5M"
        extra_args = []
        ...
        return self._run_script(
            ...
            [
                ...
                dataset,
                "--model-id",
                command.text("model_id", "gold_pivot_pattern_sequence_wavenet_option_b_5m").strip()
                or "gold_pivot_pattern_sequence_wavenet_option_b_5m",
                "--train-frac",
                str(command.number("train_frac", 0.70)),
                "--val-frac",
                str(command.number("val_frac", 0.15)),
                ...
                str(max(command.integer("branch_target_features", 0), 0)),
                "--feature-augmentation-mode",
                command.text("feature_augmentation_mode", "off").strip().lower() or "off",
                "--feature-augmentation-clip",
                str(max(command.number("feature_augmentation_clip", 8.0), 0.0)),
                "--zero-feature-names",
                command.text("zero_feature_names", "").strip(),
                "--zero-feature-file",
                command.text("zero_feature_file", "").strip(),
                "--zero-feature-groups",
                command.text("zero_feature_groups", "").strip(),
                "--temporal-filters",
                str(max(command.integer("temporal_filters", 96), 1)),
                "--temporal-kernels",
                command.text("temporal_kernels", "3,5,9").strip() or "3,5,9",
                ...
                str(self._storage_root),
                "--output-dir",
                "run_logs\\pivot_pattern_sequence_wavenet_option_b",
                "--report-title",
                command.text("report_title", "Option B Pivot Sequence WaveNet training").strip()
                or "Option B Pivot Sequence WaveNet training",
                *extra_args,
            ],
            f"Trained Phase147A Option B pivot sequence WaveNet for {symbol} {dataset}",
            started,
            timeout=max(command.integer("timeout_minutes", 720), 5) * 60,
        )
    ...
    def backtest_pivot_pattern_sequence_wavenet_range(self, command: Command) -> CommandResult:
        """Run Phase148A range-aware sequence WaveNet archive/backtest."""
        started = time.monotonic()
        symbol = command.text("symbol", "XAUUSD").strip().upper() or "XAUUSD"
        dataset = command.text("dataset", "5M").strip().upper() or "5M"
        extra_args = []
        ...
        return self._run_script(
            ...
            [
                ...
                str(command.number("min_sell_r", -999.0)),
                "--range-source",
                command.text("range_source", "auto").strip().lower() or "auto",
                "--range-bracket-mode",
                command.text("range_bracket_mode", "range_capped").strip().lower()
                or "range_capped",
                "--range-fallback",
                command.text("range_fallback", "skip").strip().lower() or "skip",
                "--range-1d-model-id",
                command.text("range_1d_model_id", "gold_range_1d").strip() or "gold_range_1d",
                "--range-4h-model-id",
                command.text("range_4h_model_id", "gold_range_4h").strip() or "gold_range_4h",
                ...
                str(max(command.integer("range_4h_version", 0), 0)),
                "--use-1d-tp-cap",
                "1" if command.text("use_1d_tp_cap", "1").strip() != "0" else "0",
                "--min-range-4h-room",
                str(max(command.number("min_range_4h_room", 0.0), 0.0)),
                "--min-range-1d-room",
                str(max(command.number("min_range_1d_room", 0.0), 0.0)),
                "--min-tp-distance",
                str(max(command.number("min_tp_distance", 0.5), 0.0)),
                "--min-sl-distance",
                str(max(command.number("min_sl_distance", 0.5), 0.0)),
                "--max-tp-atr",
                str(max(command.number("max_tp_atr", 2.0), 0.0)),
                "--max-sl-atr",
                str(max(command.number("max_sl_atr", 1.25), 0.0)),
                "--atr-tp-multiplier",
                str(max(command.number("atr_tp_multiplier", 0.75), 0.01)),
                "--atr-sl-multiplier",
                str(max(command.number("atr_sl_multiplier", 0.75), 0.01)),
                "--hold-bars",
                str(max(command.integer("hold_bars", 48), 1)),
                "--spread-mode",
                command.text("spread_mode", "pct").strip().lower() or "pct",
                ...
                str(self._storage_root),
                "--output-dir",
                "run_logs\\pivot_pattern_sequence_wavenet_range_archive",
                "--report-title",
                command.text("report_title", "Phase148A range-aware sequence WaveNet archive").strip()
                or "Phase148A range-aware sequence WaveNet archive",
                *extra_args,
            ],
            f"Built Phase148A range-aware archive for {symbol} {dataset}",
            started,
            timeout=max(command.integer("timeout_minutes", 360), 5) * 60,
        )

    def audit_pivot_sequence_range_bracket_geometry(self, command: Command) -> CommandResult:
        """Run Phase150A range-bracket geometry audit from archived predictions."""
        started = time.monotonic()
        return self._run_script(
            command,
            [
                ...
                or "run_logs/pivot_pattern_sequence_wavenet_range_archive_validation/latest_predictions.parquet",
                "--flat-path",
                command.text(
                    "flat_path",
                    "datasets/processed/XAUUSD/5M/pivot_pattern_sequence_flat_latest.parquet",
                ).strip()
                or "datasets/processed/XAUUSD/5M/pivot_pattern_sequence_flat_latest.parquet",
                "--split-name",
                command.text("split_name", "validation").strip() or "validation",
                "--allowed-sides",
                command.text("allowed_sides", "BUY,SELL").strip() or "BUY,SELL",
                "--range-bracket-modes",
                command.text("range_bracket_modes", "atr,range_capped").strip() or "atr,range_capped",
                "--range-fallbacks",
                command.text("range_fallbacks", "skip").strip() or "skip",
                "--use-1d-tp-caps",
                command.text("use_1d_tp_caps", "1,0").strip() or "1,0",
                "--min-tp-distances",
                command.text("min_tp_distances", "0.5,2,5").strip() or "0.5,2,5",
                "--min-sl-distances",
                command.text("min_sl_distances", "0.5,2,5,10").strip() or "0.5,2,5,10",
                "--max-tp-atrs",
                command.text("max_tp_atrs", "2.0,0").strip() or "2.0,0",
                "--max-sl-atrs",
                command.text("max_sl_atrs", "1.25,2.0").strip() or "1.25,2.0",
                "--min-range-4h-rooms",
                command.text("min_range_4h_rooms", "0").strip() or "0",
                "--min-range-1d-rooms",
                command.text("min_range_1d_rooms", "0").strip() or "0",
                "--same-bar-policies",
                command.text("same_bar_policies", "stop_first").strip() or "stop_first",
                "--atr-tp-multiplier",
                str(max(command.number("atr_tp_multiplier", 0.75), 0.01)),
                "--atr-sl-multiplier",
                str(max(command.number("atr_sl_multiplier", 0.75), 0.01)),
                ...
                str(max(command.number("risk_per_trade", 0.01), 0.0)),
                "--min-trades",
                str(max(command.integer("min_trades", 30), 1)),
                "--score-metric",
                command.text("score_metric", "drawdown_adjusted").strip() or "drawdown_adjusted",
                "--pass-min-profit-factor",
                str(max(command.number("pass_min_profit_factor", 1.05), 0.0)),
                "--pass-min-final-balance",
                str(max(command.number("pass_min_final_balance", 100.0), 0.0)),
                "--pass-max-drawdown",
                str(max(command.number("pass_max_drawdown", 25.0), 0.0)),
                "--output-dir",
                command.text("output_dir", "run_logs/pivot_sequence_range_bracket_geometry").strip()
                or "run_logs/pivot_sequence_range_bracket_geometry",
                "--report-title",
                command.text("report_title", "Phase150A range bracket geometry audit").strip()
                or "Phase150A range bracket geometry audit",
            ],
            "Audited Phase150A range-bracket geometry",
            started,
            timeout=max(command.integer("timeout_minutes", 240), 5) * 60,
        )

    def run_pivot_sequence_option_b_seed_transfer(self, command: Command) -> CommandResult:
        """Run Phase151A Option B seed repeatability / transfer validation."""
        started = time.monotonic()
        symbol = command.text("symbol", "XAUUSD").strip().upper() or "XAUUSD"
        dataset = command.text("dataset", "5M").strip().upper() or "5M"
        extra_args = []
        ...
        return self._run_script(
            ...
            [
                ...
                dataset,
                "--model-id-prefix",
                command.text("model_id_prefix", "gold_pivot_pattern_sequence_wavenet_option_b_seed_audit_5m").strip()
                or "gold_pivot_pattern_sequence_wavenet_option_b_seed_audit_5m",
                "--seeds",
                command.text("seeds", "20260919,20260920,20260921").strip()
                or "20260919,20260920,20260921",
                "--max-samples",
                str(max(command.integer("max_samples", 24000), 0)),
                "--train-frac",
                str(command.number("train_frac", 0.70)),
                ...
                str(max(command.integer("hold_bars", 48), 1)),
                "--selection-metric",
                command.text("selection_metric", "drawdown_adjusted").strip() or "drawdown_adjusted",
                "--validation-min-trades",
                str(max(command.integer("validation_min_trades", 30), 1)),
                "--validation-min-profit-factor",
                str(max(command.number("validation_min_profit_factor", 1.05), 0.0)),
                "--validation-min-final-balance",
                str(max(command.number("validation_min_final_balance", 100.0), 0.0)),
                "--test-min-profit-factor",
                str(max(command.number("test_min_profit_factor", 1.05), 0.0)),
                "--test-min-final-balance",
                str(max(command.number("test_min_final_balance", 100.0), 0.0)),
                "--storage-root",
                str(self._storage_root),
                "--output-dir",
                command.text("output_dir", "run_logs/pivot_sequence_option_b_seed_transfer").strip()
                or "run_logs/pivot_sequence_option_b_seed_transfer",
                "--skip-existing-models",
                "0" if command.text("skip_existing_models", "1").strip() == "0" else "1",
                "--skip-training",
                "1" if command.text("skip_training", "0").strip() == "1" else "0",
                *extra_args,
            ],
            "Ran Phase151A Option B seed transfer validation",
            started,
            timeout=max(command.integer("timeout_minutes", 720), 5) * 60,
        )

    def audit_pivot_label_target_stability(self, command: Command) -> CommandResult:
        """Run Phase152A pivot label / target stability audit."""
        started = time.monotonic()
        symbol = command.text("symbol", "XAUUSD").strip().upper() or "XAUUSD"
        dataset = command.text("dataset", "5M").strip().upper() or "5M"
        extra_args = []
        ...
        return self._run_script(
            ...
            [
                ...
                command.text("sensitivity_lookahead_bars", "24,48,72").strip() or "24,48,72",
                "--sensitivity-pivot-move-atrs",
                command.text("sensitivity_pivot_move_atrs", "0.5,0.75,1.0").strip() or "0.5,0.75,1.0",
                "--sensitivity-pivot-zone-atrs",
                command.text("sensitivity_pivot_zone_atrs", "0.25,0.35,0.5").strip() or "0.25,0.35,0.5",
                "--sensitivity-min-direction-edges",
                command.text("sensitivity_min_direction_edges", "1.0,1.1,1.25").strip() or "1.0,1.1,1.25",
                "--max-sensitivity-combinations",
                str(max(command.integer("max_sensitivity_combinations", 40), 0)),
                "--psi-warning-threshold",
                str(max(command.number("psi_warning_threshold", 0.20), 0.0)),
                "--storage-root",
                str(self._storage_root),
                "--output-dir",
                command.text("output_dir", "run_logs/pivot_label_target_stability").strip()
                or "run_logs/pivot_label_target_stability",
                "--report-title",
                command.text("report_title", "Phase152A pivot label target stability audit").strip()
                or "Phase152A pivot label target stability audit",
                *extra_args,
            ],
            "Audited Phase152A pivot label/target stability",
            started,
            timeout=max(command.integer("timeout_minutes", 240), 5) * 60,
        )

    def run_pivot_target_redesign_candidate_audit(self, command: Command) -> CommandResult:
        """Run Phase153A pivot target redesign candidate build/audit."""
        started = time.monotonic()
        symbol = command.text("symbol", "XAUUSD").strip().upper() or "XAUUSD"
        dataset = command.text("dataset", "5M").strip().upper() or "5M"
        return self._run_script(
            ...
            [
                ...
                or "C1:stable:24:0.75:0.25:1.0;C2:dense:24:0.5:0.5:1.25",
                "--recent-window-bars",
                str(max(command.integer("recent_window_bars", 48), 1)),
                "--window-size",
                str(max(command.integer("window_size", 100), 2)),
                ...
                str(max(command.integer("sample_stride", 1), 1)),
                "--build-max-samples",
                str(max(command.integer("build_max_samples", 0), 0)),
                "--audit-max-samples",
                str(max(command.integer("audit_max_samples", 24000), 0)),
                "--include-source-5m-features",
                "1" if command.text("include_source_5m_features", "1").strip() != "0" else "0",
                "--source-5m-feature-path",
                command.text("source_5m_feature_path", "").strip(),
                ...
                str(max(command.number("max_tensor_mb", 8192.0), 1.0)),
                "--health-full-scan",
                "1" if command.text("health_full_scan", "1").strip() != "0" else "0",
                "--health-max-scan-samples",
                str(max(command.integer("health_max_scan_samples", 0), 0)),
                "--train-frac",
                str(command.number("train_frac", 0.70)),
                "--val-frac",
                str(command.number("val_frac", 0.15)),
                ...
                str(self._storage_root),
                "--output-dir",
                command.text("output_dir", "run_logs/pivot_target_redesign_candidates").strip()
                or "run_logs/pivot_target_redesign_candidates",
                "--skip-existing-tensors",
                "1" if command.text("skip_existing_tensors", "0").strip() == "1" else "0",
                "--report-title",
                command.text(
                    "report_title", "Phase153A pivot target redesign candidate build/audit"
                ).strip()
                or "Phase153A pivot target redesign candidate build/audit",
            ],
            "Ran Phase153A pivot target redesign candidate build/audit",
            started,
            timeout=max(command.integer("timeout_minutes", 720), 5) * 60,
        )

    def audit_pivot_payoff_target_redesign(self, command: Command) -> CommandResult:
        """Run Phase154A pivot payoff / trade-outcome target redesign audit."""
        started = time.monotonic()
        symbol = command.text("symbol", "XAUUSD").strip().upper() or "XAUUSD"
        dataset = command.text("dataset", "5M").strip().upper() or "5M"
        extra_args = []
        ...
        return self._run_script(
            ...
            [
                ...
                or "B1:balanced_24_rr1:24:0.35:0.75:0.75:0.0;B2:conservative_24_tp1_sl075:24:0.25:1.0:0.75:0.1;B3:dense_24_half_atr:24:0.50:0.50:0.50:0.0;B4:asym_24_tp1_sl05:24:0.35:1.0:0.50:0.1;B5:short_12_rr1:12:0.35:0.75:0.75:0.0;B6:wide_36_rr1:36:0.35:0.75:0.75:0.0",
                "--max-samples",
                str(max(command.integer("max_samples", 24000), 0)),
                "--train-frac",
                str(command.number("train_frac", 0.70)),
                ...
                command.text("same_bar_policy", "stop_first").strip().lower() or "stop_first",
                "--timeout-score-mode",
                command.text("timeout_score_mode", "zero").strip().lower() or "zero",
                "--psi-warning-threshold",
                str(max(command.number("psi_warning_threshold", 0.20), 0.0)),
                "--min-train-actionable-rate",
                str(max(command.number("min_train_actionable_rate", 0.02), 0.0)),
                "--min-validation-actionable-rate",
                str(max(command.number("min_validation_actionable_rate", 0.02), 0.0)),
                "--min-test-actionable-rate",
                str(max(command.number("min_test_actionable_rate", 0.02), 0.0)),
                "--storage-root",
                str(self._storage_root),
                "--output-dir",
                command.text("output_dir", "run_logs/pivot_payoff_target_redesign").strip()
                or "run_logs/pivot_payoff_target_redesign",
                "--report-title",
                command.text("report_title", "Phase154A pivot payoff target redesign audit").strip()
                or "Phase154A pivot payoff target redesign audit",
                *extra_args,
            ],
