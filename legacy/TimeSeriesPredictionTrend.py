...
def window_dataset(
	...
	# def TradeFilter(window, target):
	# 	if tf.math.greater(target, 1):
	# 		return 1
	# 	elif tf.math.less(target, 1):
	# 		return -1
	# 	else:
	# 		return 0

	# dataset = dataset.filter(TradeFilter)
...
if os.path.exists('model_' + feature_names_str + '_' + timeframe + '.h5'):
	...
	learning_rate = 6.3e-9 #6.3e-9 #1.5e-8
