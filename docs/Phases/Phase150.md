# Phase150 — Range Bracket Geometry Audit for Pivot Sequence Archive

**Status:** ✅ Phase150A implemented  
**Type:** Research/diagnostic bracket-policy audit  
**Production status:** BLOCKED — no paper/live

## Why this phase exists

Phase148C showed that using current range-model TP/SL geometry directly was catastrophic:

```text
Train range-aware archive:
  final_balance = 6.5163
  PF            = 0.6074
  maxDD         = 93.5009

Validation range-aware archive:
  final_balance = 27.1922
  PF            = 0.3114
  maxDD         = 76.6329
```

ATR validation for the same rebuilt model was positive, so before filtering model signals we must isolate bracket geometry:

```text
Is the prediction/archive path broken?
Or is range TP/SL geometry broken?
Which min-distance/cap/fallback policy, if any, survives validation?
```
...
The script consumes a Phase148 prediction archive; it does not retrain and does not recompute model predictions.
...
It also needs the original flat OHLCV/features file for candle replay:
...
It replays the same model-selected BUY/SELL actions under policy grids:

```text
range_bracket_modes : atr, range, range_capped
range_fallbacks     : skip, atr
use_1d_tp_caps      : 1,0
min_tp_distances    : comma-separated values
min_sl_distances    : comma-separated values
max_tp_atrs         : comma-separated values
max_sl_atrs         : comma-separated values
same_bar_policies   : stop_first,tp_first
allowed_sides       : BUY,SELL or side-specific
```
...
This is the ATR-control inside the same archive replay path.
...
Run this on validation first:

```powershell
python -u scripts\audit_pivot_sequence_range_bracket_geometry.py `
  ...
  --range-bracket-modes atr,range_capped `
  --range-fallbacks skip,atr `
  --use-1d-tp-caps 1,0 `
  --min-tp-distances 0.5,2,5 `
  --min-sl-distances 0.5,2,5,10,15 `
  --max-tp-atrs 2.0,3.0,0 `
  --max-sl-atrs 1.25,2.0,3.0,0 `
  --same-bar-policies stop_first `
  ...
  --pass-min-profit-factor 1.05 `
  --pass-min-final-balance 100 `
  --pass-max-drawdown 25 `
  --output-dir run_logs\pivot_sequence_range_bracket_geometry_validation `
  --report-title "Phase150A range bracket geometry audit VALIDATION"
```

## Anti-overfit rule

```text
Validation is for bracket-policy discovery.
Test remains confirmation-only.
If a validation-selected bracket fails test, reject it.
If it passes test, walk-forward is still required before paper/live.
```
