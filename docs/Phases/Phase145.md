# Phase145 — Pivot Sequence Tensor + Advanced Conv1D WaveNet

**Status:** ✅ Phase145A implemented  
**Type:** Option A sequence model / Conv1D-WaveNet research  
...
The owner proposed simplifying the 4D image tensor into the natural WaveNet/Conv1D layout:

```text
Stored tensor: [samples, WindowSize, Features]
Keras batch : [batch, WindowSize, Features]
```

This keeps all 5M/4H/1D features on one feature/channel axis instead of expanding 5M × HTF interactions into a 4D matrix.

Expected advantages:

```text
Much lower RAM and disk pressure
More natural for Conv1D/WaveNet
Can include all available 5M source features, including the historical ~188-feature dataset
No artificial 2D feature adjacency assumption
Simpler normalization than 4D interaction tensors
Potentially better validation generalization
```
...
```text
X = [samples, 100, features]
```

Keras batch:

```text
batch_X = [batch, 100, features]
```
...
source 5M numeric feature columns from project parquet, prefixed src5m_
```

If the operator's stored 5M parquet contains the historical larger feature set, those columns are included with:

```text
--include-source-5m-features 1
```

`--max-features 0` means keep all selected features.

---

## Targets

Targets are the same pivot targets used by the 4D model:
...
This makes Phase145 comparable to Phase143/144 while reducing dimensionality.
...
run_logs\pivot_pattern_sequence_tensor\latest.html
```
...
Architecture mirrors the advanced 4D Image WaveNet, adapted to a sequence feature axis:

```text
Input [WindowSize, Features]
│
├── 5M branch
│   └── gather generated_5m + source_5m + session features
│   └── multi-scale causal Conv1D kernels 3,5,9
│
├── HTF branch
│   └── gather closed_4h + closed_1d features
│   └── multi-scale causal Conv1D kernels 3,5,9
│
├── all-feature branch
│   └── multi-scale causal Conv1D kernels 3,5,9
...
```powershell
python -u scripts/train_pivot_pattern_sequence_wavenet.py `
  --symbol XAUUSD `
  --timeframe 5M `
  ...
  --save-model 1 `
  --storage-root datasets `
  --output-dir run_logs\pivot_pattern_sequence_wavenet `
  --report-title "Advanced Pivot Sequence WaveNet training"
```

Outputs:

```text
datasets\models\gold_pivot_pattern_sequence_wavenet_5m\vN_model.keras
datasets\models\gold_pivot_pattern_sequence_wavenet_5m\vN_training.json
datasets\models\gold_pivot_pattern_sequence_wavenet_5m\vN_architecture.json
datasets\models\gold_pivot_pattern_sequence_wavenet_5m\vN_summary.txt
datasets\models\gold_pivot_pattern_sequence_wavenet_5m\vN_epoch_checkpoint.keras
run_logs\pivot_pattern_sequence_wavenet\latest.json
run_logs\pivot_pattern_sequence_wavenet\latest.html
run_logs\pivot_pattern_sequence_wavenet\latest_summary.txt
run_logs\pivot_pattern_sequence_wavenet\latest_architecture.json
run_logs\pivot_pattern_sequence_wavenet\latest_training_log.csv
run_logs\pivot_pattern_sequence_wavenet\latest_batch_log.jsonl
```
...
Internal TESTSYM smoke for the builder:
...
On the operator's real XAUUSD dataset, if extra 5M feature columns exist, the feature count should be higher.
...
```text
sequence tensor builder exists
advanced Conv1D/WaveNet trainer exists
GUI commands exist
tests/docs/owner map updated
```

Trading acceptance requires actual train/backtest/walk-forward results. Until then:
