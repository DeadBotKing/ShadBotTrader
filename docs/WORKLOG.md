...
مدل سیگنال از سه‌کلاسهٔ `SELL/HOLD/BUY` به طبقه‌بندی binary تغییر کرد. خروجی شبکه و labelها فقط `SELL` و `BUY` هستند؛ `HOLD` اگر لازم باشد فقط در لایهٔ strategy/risk به‌عنوان `no_trade` ساخته می‌شود و کلاس مدل نیست. threshold قدیمیِ neutral band دیگر روی labelها اثر ندارد و مدل‌های قدیمی سه‌خروجی باید دوباره آموزش داده شوند.


## 2026-08-19 — حذف کلاس HOLD از مدل Signal

مدل سیگنال به طبقه‌بندی binary تغییر کرد: خروجی شبکه فقط دو احتمال `SELL` و `BUY` دارد و `HOLD` از target، label builder، predictor و نمایش probability حذف شد. اگر probability از آستانه پایین‌تر باشد، استراتژی تصمیم `no_trade/HOLD` ثبت می‌کند؛ این تصمیم مدل نیست و در لایهٔ strategy انجام می‌شود. مدل‌های قدیمی سه‌کلاسه برای این قرارداد معتبر نیستند و باید دوباره آموزش داده شوند.
